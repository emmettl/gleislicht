...
function publicAppAssets(baseUrl: string): Plugin {
  let root: string, output: string
  return {
    name: 'gleislicht-public-app-assets',
    apply: 'build',
    generateBundle() { this.emitFile({ type: 'asset', fileName: '_data-release.json', source: JSON.stringify({ ...release, baseUrl }) + '\n' }) },
...
export default defineConfig(({ command, mode }) => {
  ...
  return {
    ...
    plugins: [...(bundledData ? [] : [publicAppAssets(dataRoot)]), gleislichtSelectionRenderer(), gleislichtRoadRenderer(), gleislichtAirportRenderer(), gleislichtSurfaceRenderer(), gleislichtPostbusRenderer(), gleislichtPerformanceRenderer(), gleislichtOrbitalRenderer(), react()],
    optimizeDeps: {
