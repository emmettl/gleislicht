...
import { PORTALBAN_PAIRS } from './fribourg-portalban.mjs'
import { execFileSync } from 'node:child_process'
import { readFile, writeFile } from 'node:fs/promises'

const baseline = 'ac4d685'
...
for (const date of ['2026-09-04', '2026-09-06']) {
  ...
  assert(added.every(t => t.roadReviewKinds?.includes('portalban-school-streets')), 'New journey outside Portalban evidence')
  assert.equal(added.length, date === '2026-09-04' ? 12 : 0)
  assert.deepEqual([...new Set(added.map(t => t.directionId))].sort(), date === '2026-09-04' ? ['0', '1'] : [])
  for (const t of added) {
    assert.equal(t.routeId, '92-544-j26-1')
    const stopIds = t.stops.map(([i]) => b.stops[i][4])
    const matches = PORTALBAN_PAIRS.flatMap((pair, p) => stopIds.flatMap((id, i) => id === pair[0] && stopIds[i + 1] === pair[1] ? [{ p, i }] : []))
    assert.equal(matches.length, 1)
    const { p, i } = matches[0]
    assert.equal(t.directionId, p === 0 ? '0' : '1')
    assert.equal(t.stops[i + 1][1] - t.stops[i][2], 60, 'Changed original one-minute Portalban interval')
  }
  ...
  days.push({ date, previousJourneys: previous.size, presentJourneys: present.size, addedJourneys: added.length,
    ...
    newJourneys: added.map(t => ({ id: t.id, routeId: t.routeId, sourceServiceDate: t.sourceServiceDate, directionId: t.directionId,
      reviewedCalls: t.stops.filter(([i]) => PORTALBAN_PAIRS.flat().includes(b.stops[i][4])).map(([i, ...times]) => [b.stops[i], ...times]), reviewKinds: t.roadReviewKinds })) })
}
const summary = await current('data/fribourg-audit/summary.json')
await writeFile('data/fribourg-audit/portalban-regression.json', JSON.stringify({ baselineCommit: execFileSync('git', ['rev-parse', baseline]).toString().trim(), sourceHashes: summary.sourceHashes,
  assertions: 'Every previously admitted journey retains its original stop identity, coordinates, call permissions, times, direction and segment geometry. Added journeys must carry reviewed Portalban road evidence.', days }, null, 2) + '\n')
