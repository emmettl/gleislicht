...
d.text((28,68),'Blue: admitted platform-3 path. Orange: rejected permissive platform-2 candidate. No basemap.',font=font(19),fill='#354f64')
for col,(title,points) in enumerate([('Whole border leg',accepted+rejected),('Rejected junction reversal',[[turns[0][0]-.002,turns[0][1]-.002],[turns[0][0]+.002,turns[0][1]+.002]])]):
 ...
 for path,colour in [(rejected,'#c8692b'),(accepted,'#007f9b')]:pd.line([project(p) for p in path],fill=colour,width=3)
 for p in turns:
...
d.text((28,839),'Red circle: forbidden sharp turn. Physical track selection, signalling and operational direction remain unverified.',font=font(18),fill='#354f64')
