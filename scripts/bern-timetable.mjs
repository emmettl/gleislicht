...
export function bernInstances(id, source, date, calendars, intervals) {
  const result = []
  for (const offset of [0, -86400]) {
    if (!calendars.get(offset ? previousServiceDate(date) : date).has(source.serviceId)) continue
    const instances = intervals ? expandFrequencyTrip(id, source.calls, intervals, -offset, 86400 - offset)
      : [{ id, stops: source.calls }]
    for (const instance of instances) {
      const civil = civilTripInstance(instance.id, instance.stops, { sourceTripId: id, ...(instance.frequency ? { frequency: instance.frequency } : {}) }, offset, date)
      const start = civil.stops[0]?.departure, end = civil.stops.at(-1)?.arrival
      if (civil.stops.length < 2 || start >= 86400 || end < 0) continue
      result.push({ id: civil.id, ...civil.metadata, sourceTripId: id, start, end, calls: civil.stops })
    }
  }
  return result
}
...
export async function readBernTimetables(archive, dates, source) {
  ...
  const snapshots = dates.map(date => {
    ...
    assert(new Set(trains.map(t => t.id)).size === trains.length, 'Duplicate Bern civil instance')
