...
describe('TfL rail-led catalogue', () => {
  ...
  it('keeps the committed discovery audit complete and capability-honest', async () => {
    ...
    expect(catalogue.movementProofs.find(({ mode }) => mode === 'elizabeth-line').status).toBe('topology-only')
    expect(catalogue.movementProofs.find(({ mode }) => mode === 'overground').status).toBe('topology-only')
  })
})
