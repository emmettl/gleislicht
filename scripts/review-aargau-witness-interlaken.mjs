...
const prior=await readGzipJson(`${root}/rail-review-patterns.json.gz`),priorSummary=await readJson(`${root}/rail-review-summary.json`)
const context=await loadSeasonalContext('data/aargau-seasonal/input'),rail=await loadWitnessRail(),interlaken=await loadWitnessInterlaken()
assert.equal(priorSummary.policySha256,await hashFile('data/aargau-witness-rail-policy.json'))
assert.equal(priorSummary.geometryPatternsSha256,await hashFile(`${root}/rail-review-patterns.json.gz`))
...
for(const t of raw.trains){const stops=t.calls.map(c=>raw.stops.find(s=>s[4]===c[0]));rail.assertTemplate(t,stops);interlaken.assertTemplate(t,stops)}
const rails={matchPattern:(t,stops)=>context.rails.matchPattern(t,stops)??rail.matchPattern(t,stops)}
const before=applyAargauGeometry(raw,context.index,context.inventory.cantonStopIds,context.roads,rails)
...
const after=applyAargauGeometry(raw,context.index,context.inventory.cantonStopIds,context.roads,rails,interlaken)
const regression=assertPriorGeometryPreserved(before,after)
assert.equal(regression.preservedOccurrences,priorSummary.compatibleOccurrences);assert.equal(regression.addedOccurrences,interlaken.policy.expected.addedOccurrences)
...
const oldById=new Map(prior.patterns.map(p=>[p.id,p])),ruleById=new Map(interlaken.policy.patterns.map(p=>[p.id,p])),unresolved=[],failures={},addedByDirection={}
...
for(const p of after.patterns)for(const [i,s] of p.segments.entries()){
 ...
 if(s.witnessInterlakenPatternId){
  assert.equal(s.witnessInterlakenPatternId,p.id);assert.equal(old.pathIndex,null)
...
const summary={schemaVersion:1,policySha256:await hashFile(INTERLAKEN_POLICY),previousReviewSha256:await hashFile(`${root}/rail-review-summary.json`),previousPatternsSha256:await hashFile(`${root}/rail-review-patterns.json.gz`),inventorySha256:await hashFile(`${root}/inventory.json`),sourceVerificationSha256:await hashFile(`${root}/source-verification.json`),geometryPatternsFile:'interlaken-review-patterns.json.gz',geometryPatternsSha256:await hashFile(file),
 scope:'Separate annual witness-template candidate. Exact FOT parent/track-group relation supplies only previously absent geometry at two Interlaken Ost platforms. Every prior accepted path and every other source assessment stays identical. Full GTFS IDs, coordinates, calendars and calls are preserved. These are archived trip-template counts, not date-specific regional feeds or certified running tracks.',
 attribution:priorSummary.attribution,source:interlaken.policy.source,hierarchy:interlaken.policy.hierarchy,regression:{...regression,completeSourceTemplatesAndCallsPreserved:true,allOtherAssessmentsPreserved:true,previousCandidateReplayed:true},addedByDirection,
...
if(process.argv.includes('--check'))assert.deepEqual(await readJson(`${root}/interlaken-review-summary.json`),summary)
else await writeFile(`${root}/interlaken-review-summary.json`,JSON.stringify(summary,null,2)+'\n')
