...
import { SCHUPFART_POLICY,loadWitnessSchupfart } from './aargau-witness-schupfart.mjs'
...
const prior=await readGzipJson(`${root}/oberentfelden-review-patterns.json.gz`),priorSummary=await readJson(`${root}/oberentfelden-review-summary.json`)
const context=await loadSeasonalContext('data/aargau-seasonal/input'),rail=await loadWitnessRail(),interlaken=await loadWitnessInterlaken(),edges=await loadWitnessEdges(),ava=await loadWitnessAva(),ober=await loadWitnessOberentfelden(),schup=await loadWitnessSchupfart()
assert.equal(priorSummary.policySha256,await hashFile('data/aargau-witness-oberentfelden-policy.json'))
assert.equal(priorSummary.geometryPatternsSha256,await hashFile(`${root}/oberentfelden-review-patterns.json.gz`))
...
for(const t of raw.trains){const stops=t.calls.map(c=>raw.stops.find(s=>s[4]===c[0]));rail.assertTemplate(t,stops);interlaken.assertTemplate(t,stops);edges.assertTemplate(t,stops);ava.assertTemplate(t,stops);ober.assertTemplate(t,stops);schup.assertTemplate(t,stops)}
...
const platforms={matchPattern:(t,stops)=>previousPlatforms.matchPattern(t,stops)??schup.matchPattern(t,stops)}
...
assert.equal(regression.preservedOccurrences,priorSummary.compatibleOccurrences);assert.equal(regression.addedOccurrences,schup.policy.expected.addedOccurrences)
...
const oldById=new Map(prior.patterns.map(p=>[p.id,p])),ruleById=new Map(schup.policy.patterns.map(p=>[p.id,p])),unresolved=[],failures={},addedByDirection={}
...
for(const p of after.patterns)for(const [i,s] of p.segments.entries()){
 ...
 if(s.witnessSchupfartPatternId){
  assert.equal(s.witnessSchupfartPatternId,p.id);assert.equal(old.pathIndex,null)
...
const summary={schemaVersion:1,policySha256:await hashFile(SCHUPFART_POLICY),previousReviewSha256:await hashFile(`${root}/oberentfelden-review-summary.json`),previousPatternsSha256:await hashFile(`${root}/oberentfelden-review-patterns.json.gz`),inventorySha256:await hashFile(`${root}/inventory.json`),sourceVerificationSha256:await hashFile(`${root}/source-verification.json`),geometryPatternsFile:'schupfart-review-patterns.json.gz',geometryPatternsSha256:await hashFile(file),
 scope:'Separate annual witness-template candidate. The official Schupfart Festival 2026 timetable verifies all 32 exact PostAuto event templates at advertised major calls. Admit 195 inferred OSM road occurrences while holding 12 zero-second adjacent-call intervals. Every prior accepted path, other assessment, full GTFS identity, coordinate, calendar and call remains unchanged. These are archived trip-template counts, not additional daily feeds or certified operating itineraries.',
 attribution:priorSummary.attribution,source:schup.policy.source,schupfartDispositionOccurrences:schup.policy.expected.occurrences,regression:{...regression,completeSourceTemplatesAndCallsPreserved:true,allOtherAssessmentsPreserved:true,previousCandidateReplayed:true},addedByDirection,
...
assert.equal(summary.compatibleOccurrences,8298);assert.equal(summary.busMissingOccurrences,2690);assert.equal(summary.fullyCompatiblePatterns,201)
if(process.argv.includes('--check'))assert.deepEqual(await readJson(`${root}/schupfart-review-summary.json`),summary)
else await writeFile(`${root}/schupfart-review-summary.json`,JSON.stringify(summary,null,2)+'\n')
