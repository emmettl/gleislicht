...
const directory = 'data/solothurn-access-roads', input = 'data/solothurn-pattern-contexts.json.gz'
const raw = JSON.parse(gunzipSync(await readFile(input)))
const routeIds = ['92-501-A-j26-1', '96-131-1-j26-1', '92-EV4-M-j26-1', '92-EV6-R-j26-1', '92-M11-j26-1', '92-M53-j26-1', '92-N22-j26-1', '96-131-8-j26-1']
...
const metadata = { serviceDate: raw.snapshots[0].metadata.serviceDate, dates: raw.snapshots.map(d => d.metadata.serviceDate), feedVersion: raw.snapshots[0].metadata.feedVersion,
  ...
  scope: 'Every complete retained twelve-date pattern on eight routes with unresolved bus pairs on the two published dates. Original calls and coordinates retained; routing-only nonnegative times. Geometry experiment, not automatic admission.' }
const feed = '/private/tmp/solothurn-access-feed', osm = '/private/tmp/solothurn-access-network.osm'
...
console.log('Prepared Solothurn access-road graph and complete routing input')
