...
export function mappedRochersRoutes(network, source, evidence) {
 ...
 if(source.archiveSha256!=='75086b5aa7e721f5ad2ea080e14e9e3f42d5e0afdee31c2e3c162f412fab4114' || createHash('sha256').update(JSON.stringify(source.features)).digest('hex')!=='07bac208a6f7976f0a66bc3abc0c12a8e3cd628f5d94ee26903295ab7ffa0e49')throw new Error('Pinned Rochers railway source changed; review required')
 ...
 for(const t of network.trains){
  ...
  if(!calls || calls.length!==t.stops.length || t.stops.some(([i,a,d],j)=>network.stops[i][4]!==calls[j].stopId || a!==calls[j].arrival || d!==calls[j].departure))throw new Error('Changed source calls require a separate terrain audit')
 }
 const trains=network.trains.filter(t=>t.routeId==='91-37-F-j26-1' && t.agencyId==='131' && t.routeType===106 && network.stops[t.stops[0][0]][4]==='ch:1:sloid:1300:3:8' && network.stops[t.stops.at(-1)[0]][4]==='ch:1:sloid:1369' && evidence.trips[t.id]).sort((a,b)=>b.stops.length-a.stops.length || a.start-b.start)
 if(trains.length!==10)throw new Error('Expected all 10 dated summit ascents')
 ...
 const descents=network.trains.filter(t=>t.routeId==='91-37-F-j26-1' && t.agencyId==='131' && t.routeType===106 && network.stops[t.stops[0][0]][4]==='ch:1:sloid:1369' && network.stops[t.stops.at(-1)[0]][4]==='ch:1:sloid:1300:3:8' && evidence.trips[t.id]).sort((a,b)=>a.start-b.start)
 if(descents.length!==10)throw new Error('Expected all 10 dated complete descents')
