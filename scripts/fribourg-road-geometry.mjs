...
import { loadPortalban } from './fribourg-portalban.mjs'
import { loadBulle } from './fribourg-bulle.mjs'
...
export async function loadFribourgRoads(timetable, policy, { verifyEvidence = false } = {}) {
  ...
  const portalban = policy.portalban ? await loadPortalban(cache, boltigen?.candidates ?? broc?.candidates ?? laupen?.candidates ?? jongny?.candidates ?? review?.candidates ?? baseline, policy.portalban) : undefined
  const bulle = policy.bulle ? await loadBulle(cache, portalban?.candidates ?? boltigen?.candidates ?? broc?.candidates ?? laupen?.candidates ?? jongny?.candidates ?? review?.candidates ?? baseline, policy.bulle) : undefined
  return { ...(bulle ? { bulle: bulle.audit } : {}), ...(portalban ? { portalban: portalban.audit } : {}), ...(boltigen ? { boltigen: boltigen.audit } : {}), ...(broc ? { broc: broc.audit } : {}), ...(laupen ? { laupen: laupen.audit } : {}), candidates: bulle?.candidates ?? portalban?.candidates ?? boltigen?.candidates ?? broc?.candidates ?? laupen?.candidates ?? jongny?.candidates ?? review?.candidates ?? baseline, ...(review ? { montCarmel: review.audit } : {}), ...(jongny ? { jongny: jongny.audit } : {}), metadata: cache.metadata, policy,
