...
export async function buildLuzernRegion({ timetablePath, sourceDirectory, policyPath, output, auditPath }) {
  ...
  if (policy.roadFallback) {
    ...
    roadContexts = reviewedRoadContexts(roadCache, policy.limits, policy.roadPatternReviews)
  ...
  if (accessRoads) sourceHashes.accessRoads = policy.accessRoadFallback.sha256
  ...
  for (const day of raw.snapshots) {
    ...
    for (const train of day.trains) {
      ...
      if (!patterns.has(patternKey)) {
        const roadPatternId = route.mode === 'bus' && roadContexts.size ? luzernRoadPatternId(train, stops) : undefined
        const pairKeys = train.calls.slice(1).map((call, i) => {
          const from = train.calls[i].id, to = call.id, baseKey = JSON.stringify([route.routeId, from, to])
          const contextKey = roadContextKey(route.routeId, from, to, roadPatternId)
          const context = roadContexts.get(contextKey), key = context ? contextKey : baseKey
          if (!pairCache.has(key)) {
            ...
            if (result.path) result.geometrySource = 'official-line'
              ...
              result = road.path ? { ...road, officialAssessment: result } : access ? { ...access, officialAssessment: result, roadAssessment: road } : { ...result, roadAssessment: road }
            }
            else if (route.mode === 'rail' && rail?.pairs.has(key)) {
              const fallback = rail.pairs.get(key)
              const border = !fallback.path && borderRail?.pairs.get(key)
              result = fallback.path ? { ...fallback, officialAssessment: result } : border ? { ...border, officialAssessment: result, railAssessment: fallback } : { ...result, railAssessment: fallback }
            }
    ...
    if (roadCache) {
      ...
      metadata.geometry.roadFallback = { ...roadCache.metadata, cacheSha256: sourceHashes.roads,
        ...
        consensus: 'Reusable pairs require identical accepted paths in every complete bus pattern. Explicit roadPatternReviews bind differing valid paths to their complete routing input; they are never reused outside that context.',
        patternReviews: policy.roadPatternReviews ?? [],
    ...
    if (borderRail) {
      metadata.model = 'scheduled interpolation along cantonal, federal and SBB rail/cableway alignments and inferred OSM bus roads'
      metadata.geometry.license = 'Attribution terms (cantonal, FOT and SBB alignments); ODbL-1.0 (OSM-derived bus segments)'
      metadata.attribution.push(borderRail.source.attribution)
    ...
    days.push({ date: day.date, trips: day.trains.length, admittedTrips: admitted.length, excludedTrips: day.trains.length - admitted.length,
      ...
      admittedTripsUsingAccessRoads: ps.filter(p => p.admitted && p.pairKeys.some(k => pairs.get(k).geometrySource === 'osm-access-road-inference')).reduce((n, p) => n + p.trips, 0),
      uniqueRouteStopPairs: new Set(pairList.map(p => p.basePairKey ?? p.key)).size,
  ...
  const shippingProbeFile = 'data/luzern-shipping-probe.json', shippingProbe = await readFile(shippingProbeFile)
  const report = { schemaVersion: 2, pairKeyModel: 'JSON [routeId, fromId, toId]; explicitly reviewed road contexts append the complete roadPatternId and retain basePairKey',
    ...
    ...(accessRoads ? { accessRoads: { source: accessRoads.source,
      ...
      consensusPairs: accessRoads.all.size, acceptedConsensusPairs: [...accessRoads.all.values()].filter(p => p.path).length, reviewedPairs: [...accessRoads.pairs.keys()],
      rejectedConsensusPairs: [...accessRoads.all].filter(([, p]) => !p.path).map(([key, p]) => ({ key, ...p })) } } : {}),
