...
for (const day of raw.snapshots) {
  const result = { date: day.date, candidates: day.trains.length, baselineAdmitted: 0, admitted: 0, added: 0, addedHeadwayInstances: 0, addedScheduledInstances: 0, preservedJourneys: 0, preservedNonMountainPatterns: 0 }
  const seen = new Set()
  for (const t of day.trains) {
    ...
    if (!memo.has(key)) {
      ...
      memo.set(key, { id: sha256(key).slice(0, 20), routeId: t.routeId, train: t, before, after })
    }
