...
export async function buildLausanneDay({ archive, railPath, date, output, snapshotPath, busCachePath = 'data/lausanne-road-cache.json' }) {
  const staged = await mkdtemp(join(tmpdir(), 'lausanne-publish-'))
  try {
    const report = await auditLausanneStudy({ archive, railPath, date, output: staged, snapshotPath, busCachePath })
    ...
    for (const name of ['lausanne-region-day-manifest.json', 'lausanne-region-morning.json']) {
      ...
      value.metadata.note = 'Scheduled Lausanne-region transport including preceding service-day spillover. Rail follows matched FOT corridors with short platform connectors. OSM bus paths are inferred, not operator-verified routes. Unmatched segments retain stop interpolation; frequency services are representative, not exact departures. No live GPS positions.'
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  const report = await buildLausanneDay({ archive: arg('archive'), railPath: arg('rail'), date: arg('date'), output: resolve(arg('output-directory') ?? 'public/data'), snapshotPath: arg('snapshot') })
