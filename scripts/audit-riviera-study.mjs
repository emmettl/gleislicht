...
for (const [i, day] of days.entries()) {
  ...
  const { snapshot, reports: railReports } = applyRivieraRailGeometry(day, routes, corridors)
  ...
  snapshot.metadata.sourceHashes = { archive: archiveSha256, rail: hash(railBytes), snapshot: hash(bytes[i]), busCache: hash(cacheBytes) }
  ...
  snapshot.metadata.note = 'Unreleased audit candidate. Scheduled movements on inferred FOT rail paths. Road paths are inferred from retained OSM sources. Funicular geometry is deferred and remains explicitly missing; not live vehicle positions.'
  ...
  reports.push({ date: day.metadata.serviceDate, trips: day.trains.length, completeSourceJourneysChecked: day.trains.length, groups, payload, railProjection: railReports, gate, sourceHashes: snapshot.metadata.sourceHashes, feedVersion: day.metadata.feedVersion, sourceServiceDates: day.metadata.sourceServiceDates, scope: snapshot.metadata.studyScope })
}
