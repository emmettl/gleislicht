...
const BASELINE = '68b9ad13aa3d7f67763702139d3577193e00fc7b'
...
const baseRail = await loadBernIc61(), rail = await loadBernIc61Platforms(), routes = new Map(raw.routes.map(r => [r.id, r]))
assert.equal(summary.sourceHashes.ic61PlatformsPolicy, rail.metadata.policySha256)
...
const reviewedCrosswalk = bernIc61Crosswalk(crosswalk, baseRail.policy, raw.snapshots.map(s => s.metadata.serviceDate))
const dates = []
for (const snapshot of raw.snapshots) {
  ...
  assert.equal(added.length, date === '2026-09-04' ? 27 : 55)
  ...
  const reproduction = applyBernRail(snapshot, applyBernRail(snapshot, applyBernGeometry({ ...snapshot, trains: originalTrains }, routes, source, reviewedCrosswalk), routes, baseRail), routes, rail)
  assert.equal(reproduction.trains.filter(t => t.admission === 'admitted').length, date === '2026-09-04' ? 28 : 63)
  ...
  const reviewed = BERN_RAIL_ROUTES.map(routeId => {
    ...
    assert.deepEqual(admittedDirectionIds, ['0', '1'])
  ...
  assert.deepEqual(reviewed.map(r => r.addedScheduledJourneys), date === '2026-09-04' ? [27] : [55])
...
const report = { schemaVersion: 1, baselineCommit: BASELINE, source: rail.metadata, dates, seasonalResultsUnchanged: true,
  scope: 'Exact SBB IC61 route, original full dated patterns and coordinates. Bern platforms 5–8 connect via bounded original station-curve sections to the unchanged station node and eastern approach. Basel terminal platforms 3/9/10 clip the original eastern curve within 200–300 metres. Original calls and timetable fields retained. Prior successful paths have precedence. Remaining Basel 14/16/19 and Münsingen attachment failures stay excluded. No current running-track or complete service claim.' }
await writeFile('data/bern-audit/ic61-platforms-followup.json', JSON.stringify(report, null, 2) + '\n')
console.log(dates.map(d => `${d.date}: ${d.previousJourneysPreserved} unchanged; +${d.addedScheduledJourneys} scheduled; IC61 ${d.routes[0].admittedJourneys}/${d.routes[0].totalJourneys}; ${d.routes[0].remainingPairs.length} remaining pairs`).join('\n'))
