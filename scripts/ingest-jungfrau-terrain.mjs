...
export async function ingestMountainTerrain(study, mapRoutes, { columns=193, budgetKiB=220, evidencePath=`data/${study}-ascent-source.json` }={}) {
 const networkBytes=await readFile(`public/data/${study}-day.json`), network=JSON.parse(networkBytes)
 const sourceBytes=await readFile(`data/${study}-terrain-source.json`), source=JSON.parse(sourceBytes)
 const routes=mapRoutes(network,source,await readJson(evidencePath))
 ...
 const stac=arg('stac','/private/tmp/jungfrau-alti-stac.json'), cache=arg('cache',`/private/tmp/${study}-alti-raster.json`)
 let item
 ...
 const width=bounds.maxEasting-bounds.minEasting,depth=bounds.maxNorthing-bounds.minNorthing,rows=Math.round((columns-1)*depth/width)+1
 ...
 const artifact={id:`${study}-ascent-terrain`,version:1,metadata:{serviceDate:network.metadata.serviceDate,feedVersion:network.metadata.feedVersion,timetableSha256:network.metadata.sources.timetable.sha256,networkSha256:hash(networkBytes),source:'swissALTIRegio + swissTLM3D',sourceCrs:'EPSG:2056 / LN02',terrainRelease:item.properties.datetime.slice(0,10),terrainUrl:asset.href,terrainChecksum:asset['file:checksum'],terrainProductUrl:'https://www.swisstopo.admin.ch/en/height-model-swissaltiregio',railSourceUrl:source.sourceUrl,railSourceSha256:hash(sourceBytes),railProductUrl:source.productUrl,model:'Mapped FOT plan alignment with heights interpolated from nearby swissTLM3D rail axes; equal horizontal/vertical scale. Only unmasked outdoor sections use 3D; tunnels, covered structures and offsets above 25 m remain in 2D. Timetable motion, not live or surveyed train positions.',attribution:'Federal Office of Transport; swisstopo. Terrain: swissALTIRegio (swisstopo and contributing national elevation datasets).',bounds,gridSpacingMetres:[width/(columns-1),depth/(rows-1)],nativeResolutionMetres:10},origin,terrain:{columns,rows,widthMetres:width,depthMetres:depth,minElevation:Math.min(...elevations),maxElevation:Math.max(...elevations),elevations},routes:routes.map(r=>({routeId:r.routeId,legIndex:r.legIndex,...(r.forwardTripIds?{forwardTripIds:r.forwardTripIds}:{}),...(r.reverseTripIds?{reverseTripIds:r.reverseTripIds}:{}),vehicle:r.vehicle ?? (r.legIndex===0?'train':'cogwheel'),points:r.matches.map(m=>[Number((m.point[0]-origin.easting).toFixed(2)),Number((origin.northing-m.point[1]).toFixed(2)),Number(m.point[2].toFixed(2))]),stops:r.stops,maskedRanges:r.maskedRanges.map(m=>({...m,reason:m.kinds.includes('Tunnel')?'tunnel':m.kinds.some(k=>!['Keine','Bruecke'].includes(k))?'covered':'alignment'}))}))}
 const bytes=JSON.stringify(artifact)
 const audit={metadata:artifact.metadata,rasterSha256:hash(JSON.stringify(raster)),gzipBytes:gzipSync(bytes).length,routes:routes.map(r=>({routeId:r.routeId,tripId:r.train.id,...(r.ascentAudit?{ascentAudit:r.ascentAudit}:{}),...(r.descentAudit?{descentAudit:r.descentAudit}:{}),points:r.matches.length,length3dMetres:r.length,maxOffsetMetres:r.maxOffset,maskedFraction:r.maskedRanges.reduce((s,m)=>s+m.end-m.start,0),sourceFeatureIds:[...new Set(r.matches.map(m=>m.id))],sourceYears:[...new Set(r.matches.map(m=>m.year))],stops:r.stops.map((s,i)=>({...s,railHeight:r.matches[r.stopIndices[i]].point[2],groundHeight:sampleElevation(raster,...r.matches[r.stopIndices[i]].point)})),maskedRanges:r.maskedRanges}))}
 await writeFile(`data/${study}-terrain-audit.json`,JSON.stringify(audit,null,2)+'\n')
 if(audit.gzipBytes>budgetKiB*1024)throw new Error(`Optional terrain exceeds ${budgetKiB} KiB gzip budget`)
 await writeFile(arg('output',`public/data/${study}-ascent-terrain.json`),bytes+'\n')
 console.log(`Wrote ${columns}×${rows} terrain; ${(audit.gzipBytes/1024).toFixed(1)} KiB gzip`)
}
if(import.meta.url===`file://${process.argv[1]}`)ingestMountainTerrain('jungfrau',mappedJungfrauRoutes).catch(e=>{console.error(e);process.exitCode=1})
