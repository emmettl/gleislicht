...
export const FRIBOURG_DATES = ['2026-09-04', '2026-09-06']
...
export async function fribourgTimetable(archive, cache) {
  const sourceBytes = await readFile('data/fribourg-sources/decoded.json.gz')
  ...
  assert.equal(source.canton[0].properties.kantonsnummer, 10)
  assert.equal(source.districts.length, 7)
  const timetable = await readBernTimetables(archive, FRIBOURG_DATES, source, {
    cantonName: 'Fribourg', boundaryBounds: [2535000, 1135000, 2605000, 1210000],
  })
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  assert(process.argv[2] && process.argv[3], 'Usage: node --max-old-space-size=8192 scripts/fribourg-timetable.mjs ARCHIVE CACHE')
  await fribourgTimetable(process.argv[2], process.argv[3])
}
