...
export async function checkStGallenTopologyRegression(baselineFeed,baselineAudit,output='data/st-gallen-topology-review.json') {
  ...
  for(const day of before.days) {
    ...
    assert(added.every(t=>t.routeId==='92-321-j26-1'),'Unexpected added route')
    assert.equal(added.length,day.date==='2026-09-04'?74:67)
    ...
    const recovered=next.directedStopPairs.filter(p=>p.geometryRepairIds?.includes('balgach-321-source-gap'))
  ...
  const report={schemaVersion:1,baselineCommit:'2351822',baselineSourceHashes:before.sourceHashes,sourceHashes:after.sourceHashes,
    ...
    validation:{passed:true,allPreviouslyAdmittedCallsAndPathsUnchanged:true,allPreviouslyMatchedPairsUnchanged:true,onlyReviewedRouteAdded:true,redistributionApproved:false,directionCertified:false}}
  await writeFile(output,JSON.stringify(report,null,2)+'\n')
  return report
}
if(process.argv[1]&&import.meta.url===pathToFileURL(resolve(process.argv[1])).href) {
  assert(process.argv[2]&&process.argv[3],'Usage: node scripts/check-st-gallen-topology-regression.mjs BASELINE_FEED_DIRECTORY BASELINE_AUDIT_JSON')
  const r=await checkStGallenTopologyRegression(process.argv[2],process.argv[3]);console.log(JSON.stringify({passed:r.validation.passed,days:r.days},null,2))
}
