...
export async function checkStGallenTopologyRegression(baselineFeed,baselineAudit,output='data/st-gallen-topology-review.json',{sharedOnly=false,anchorOnly=false}={}) {
  ...
  for(const day of before.days) {
    ...
    const expected={'92-164-C-j26-1':day.date==='2026-09-04'?58:26,
      ...(!anchorOnly?{'96-250-8-j26-1':day.date==='2026-09-04'?66:35}:{}),
      ...(!sharedOnly&&!anchorOnly?{'92-321-j26-1':day.date==='2026-09-04'?74:67}:{})}
    ...
    assert.equal(recovered.length,sharedOnly||anchorOnly?0:2)
  ...
  const report={schemaVersion:1,baselineCommit:anchorOnly?'2e0c599':sharedOnly?'7926448':'2351822',baselineSourceHashes:before.sourceHashes,sourceHashes:after.sourceHashes,
...
if(process.argv[1]&&import.meta.url===pathToFileURL(resolve(process.argv[1])).href) {
  const sharedOnly=process.argv.includes('--shared'),anchorOnly=process.argv.includes('--anchor'),args=process.argv.slice(2).filter(a=>!['--shared','--anchor'].includes(a))
  assert(!(sharedOnly&&anchorOnly))
  ...
  const r=await checkStGallenTopologyRegression(args[0],args[1],anchorOnly?'data/st-gallen-stop-anchor-review.json':sharedOnly?'data/st-gallen-shared-corridor-review.json':undefined,{sharedOnly,anchorOnly});console.log(JSON.stringify({passed:r.validation.passed,days:r.days},null,2))
}
