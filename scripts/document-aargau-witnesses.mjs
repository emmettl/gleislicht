...
const inventory = await readJson(`${root}/inventory.json`), geometry = await readJson(`${root}/rail-review-summary.json`), verification = await readJson(`${root}/source-verification.json`)
...
assert.equal(geometry.policySha256, await hashFile('data/aargau-witness-rail-policy.json'))
assert.equal(geometry.originalGeometrySummarySha256, await hashFile(`${root}/geometry-summary.json`))
assert.equal(geometry.originalGeometryPatternsSha256, await hashFile(`${root}/geometry-patterns.json.gz`))
