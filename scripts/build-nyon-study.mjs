...
import { NYON_AGENCIES, nyonRailCorridor, assertCompleteNyonCalls, applyNyonGeometry } from './nyon-geometry.mjs'
...
for await (const r of rowsFromArchive(archive, 'routes.txt')) routes.set(r.route_id, { agencyId: r.agency_id, name: r.route_short_name, mode: ['738', '741'].includes(r.agency_id) ? 'bus' : 'rail' })
...
const railBytes = await readFile(railPath), corridor = nyonRailCorridor(parseRailNetworkXtf(railBytes.toString(), 10))
const cachePaths = ['738', '741'].flatMap(id => ['weekday', 'sunday'].map(day => `data/vaud-nyon-${id}-${day}-road-cache.json`))
const cacheBytes = await Promise.all(cachePaths.map(path => readFile(path))), cache = combineRoadCaches(cacheBytes.map(b => JSON.parse(b)))
const reports = []
for (const [i, day] of days.entries()) {
  assert.deepEqual(day.metadata.agencyIds, NYON_AGENCIES)
  assert.deepEqual(day.metadata.modes, ['rail', 'bus'])
  assert.deepEqual(day.metadata.sourceServiceDates, [previousServiceDate(day.metadata.serviceDate), day.metadata.serviceDate])
  assert.equal(new Set(day.trains.map(t => t.id)).size, day.trains.length, 'Duplicate Nyon journey')
  ...
  day.trains = day.trains.map(train => {
    ...
    assertCompleteNyonCalls(train, day.stops, calls.get(train.sourceTripId), day.metadata.serviceDate)
    return { ...train, routeId }
  })
  const { snapshot, rail, bus } = applyNyonGeometry(day, routes, corridor, cache)
  const groups = summarizeVaud(snapshot, routes)
  assert(groups.length === 3 && groups.every(g => g.coverage >= .95), 'Nyon per-operator geometry gate failed')
  snapshot.metadata.sourceHashes = { archive: archiveSha256, rail: hash(railBytes), snapshot: hash(bytes[i]), ...Object.fromEntries(cacheBytes.map((b, n) => [`busCache${n}`, hash(b)])) }
  snapshot.metadata.studyScope = 'Complete NStCM, TPN and Bus Nyon-Prangins journeys, including cross-border services. Not all transport in the Nyon district; excludes mainline SBB, PostAuto and lake services.'
  snapshot.metadata.note = 'Audited candidate. Scheduled movements on inferred FOT rail and OSM road paths; not live vehicle positions. Operator and visual verification recorded separately.'
  snapshot.metadata.geometry = { ...cache.metadata, matchedSegments: bus.matched, totalSegments: bus.total }
  snapshot.metadata.railGeometry = { publisher: 'Federal Office of Transport', sha256: hash(railBytes), anchor: '8501060', matchedSegments: rail.matchedSegments, totalSegments: rail.totalSegments, maximumSnapMetres: Math.max(...rail.projectionAudit.snaps.map(s => s.snapMetres ?? 0)) }
  ...
  for (const [name, value] of [['nyon-region-day-manifest.json', manifest], ['nyon-region-morning.json', morning], ...chunks.map(c => [c.descriptor.path, c.payload])]) {
    const path = join(destination, name); await mkdir(dirname(path), { recursive: true }); await writeFile(path, JSON.stringify(value))
  }
  reports.push({ date: day.metadata.serviceDate, trips: day.trains.length, completeSourceJourneysChecked: day.trains.length, groups, payload, railProjection: rail.projectionAudit, sourceHashes: snapshot.metadata.sourceHashes, feedVersion: day.metadata.feedVersion, sourceServiceDates: day.metadata.sourceServiceDates, scope: snapshot.metadata.studyScope })
}
await writeFile(join(output, 'audit.json'), JSON.stringify({ schemaVersion: 1, agencies: NYON_AGENCIES, days: reports }, null, 2) + '\n')
