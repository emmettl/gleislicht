...
export async function checkSolothurnRegion({ output = 'public/data/solothurn-region', audit = 'data/solothurn-audit', sources = 'data/solothurn-sources' } = {}) {
  ...
  const m53Review = await json(join(audit, 'm53-review.json')), m53Baseline = await json('data/solothurn-m53-baseline.json')
  assert.deepEqual(m53Review.source, supplements.metadata.m53)
  assert.deepEqual(m53Review.sourceHashes, summary.sourceHashes); assert.deepEqual(m53Baseline.sourceHashes, summary.sourceHashes)
  assert.equal(m53Review.baselineCommit, m53Baseline.commit)
  ...
  for (const day of summary.days) {
    ...
    assert.deepEqual(accessDay.sourcePairs, report.directedPairs.filter(p => p.geometrySource === 'osm-solothurn-access-road-inference').map(({ matched: _matched, ...p }) => p))
    ...
    assert.deepEqual(terminalDay.sourcePairs, report.directedPairs.filter(p => p.geometrySource === 'fot-reviewed-bern-eastern-terminal').map(({ matched: _matched, ...p }) => p))
    ...
    assert.deepEqual(s26Day.sourcePairs, report.directedPairs.filter(p => p.geometrySource === 'fot-sbb-reviewed-s26').map(({ matched: _matched, ...p }) => p))
    ...
    assert.deepEqual(comoDay.sourcePairs, report.directedPairs.filter(p => p.geometrySource === 'osm-solothurn-como-rail-inference').map(({ matched: _matched, ...p }) => p))
    ...
    assert.deepEqual(simplonDay.sourcePairs, report.directedPairs.filter(p => p.geometrySource === 'osm-solothurn-simplon-inference').map(({ matched: _matched, ...p }) => p))
    ...
    assert.deepEqual(delleDay.sourcePairs, report.directedPairs.filter(p => p.geometrySource === 'osm-solothurn-delle-rail-inference').map(({ matched: _matched, ...p }) => p))
    ...
    assert.deepEqual(roadDetourDay.sourcePairs, report.directedPairs.filter(p => p.geometrySource === 'osm-solothurn-reviewed-road-detour').map(({ matched: _matched, ...p }) => p))
    const m53Day = m53Review.days.find(d => d.date === day.serviceDate), m53Before = m53Baseline.days.find(d => d.date === day.serviceDate)
    assert.deepEqual(m53Day.before, m53Before.coverage); assert.deepEqual(m53Day.after, report.coverage)
    assert.deepEqual(m53Day.lostAdmittedPatterns, [])
    for (const id of m53Before.admittedPatternIds) assert(report.patterns.some(p => p.id === id && p.admittedTrips))
    assert.deepEqual(m53Day.newlyAdmittedPatterns.map(p => p.id).sort(), report.patterns.filter(p => p.admittedTrips && !m53Before.admittedPatternIds.includes(p.id)).map(p => p.id).sort())
    assert.deepEqual(m53Day.sourcePairs, report.directedPairs.filter(p => p.geometrySource === 'bern-official-m53-corridor').map(({ matched: _matched, ...p }) => p))
    ...
    assert.deepEqual(busDay.sourcePairs, report.directedPairs.filter(p => p.geometrySource === 'solothurn-reviewed-bus-junction').map(({ matched: _matched, ...p }) => p))
    ...
    assert.deepEqual(railDay.sourcePairs, report.directedPairs.filter(p => p.geometrySource === 'fot-reviewed-interlaken-platforms').map(({ matched: _matched, ...p }) => p))
