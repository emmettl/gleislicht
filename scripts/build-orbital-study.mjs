...
// Only released public assets. Local archival feeds are not publication inputs.
...
export async function buildOrbitalStudy(root = 'public/data') {
  ...
  for (const { id, file } of ORBITAL_SOURCES) {
    const network = await read(file), cache = new Map()
    const trains = new Map((network.trains ?? (await Promise.all(network.chunks.map(c => read(join(dirname(file), c.path))))).flatMap(c => c.trains)).map(t => [t.id, t]))
    if (network.tripCount !== undefined && trains.size !== network.tripCount) throw new Error(`Incomplete source: ${id}`)
    sources.push({ id, file, date: network.metadata.serviceDate, feedVersion: network.metadata.feedVersion, journeys: trains.size, metadata: network.metadata })
    for (const train of trains.values()) {
      ...
      journeys.set(key, { start: train.start, end: train.end, category: ORBITAL_CATEGORIES.indexOf(train.category), knots: orbitalTrajectory(train, network, cache) })
    }
  }
  ...
  console.log(JSON.stringify({ journeys: list.length, peak: Math.max(...active), bytes: chunks.reduce((n, c) => n + c.bytes, 0) }))
  return manifest
}
