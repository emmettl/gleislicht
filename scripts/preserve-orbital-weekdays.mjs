...
export async function preserveOrbitalWeekdays(root = 'public/data') {
  const retained = []
  const { selected } = await discoverOrbitalSources(root)
  for (const { id, file: name, date } of selected) {
    if (name.startsWith('weekday/')) continue
    const manifest = JSON.parse(await readFile(join(root, name), 'utf8'))
    const prefix = `weekday/${date}/`
    for (const chunk of manifest.chunks ?? []) {
      if (!/^[a-z0-9-]+(?:\/[a-z0-9-]+)*\.json$/.test(chunk.path)) throw new Error(`${id}: unsafe weekday source path`)
    }
    for (const file of [name, ...(manifest.chunks ?? []).map(chunk => join(dirname(name), chunk.path))]) {
