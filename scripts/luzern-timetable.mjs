...
export function luzernMode(type) {
...
export async function readLuzernTimetable(archive, boundaryPath, dates) {
  const boundaryBytes = await readFile(boundaryPath), boundary = JSON.parse(boundaryBytes).feature
  assert.equal(boundary.properties.ak, 'LU')
  ...
  console.log(`Census: ${cantonStops.size} GTFS stop records in the Luzern polygon; scanning all annual stop times…`)
  ...
  for await (const row of rowsFromArchive(archive, 'trips.txt')) {
    ...
    const item = inventory.get(row.route_id) ?? { routeId: row.route_id, agencyId: route.agency_id, agency: agencies.get(route.agency_id).agency_name,
      line: route.route_short_name, longName: route.route_long_name, routeType: Number(route.route_type), mode: luzernMode(Number(route.route_type)), annualTripRecords: 0,
  ...
  return { schemaVersion: 1, feed, dates, sourceHashes: { archive: sha256(await readFile(archive)), boundary: sha256(boundaryBytes) },
    scope: { description: 'Every annual GTFS trip with at least one source stop inside the complete official Luzern polygon, across every agency and mode. Fixture trips retain every source call, including cross-canton termini. No straight-line crossing test for nonstopping through traffic.',
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  ...
  assert(archive && boundary && output, 'Usage: node scripts/luzern-timetable.mjs ARCHIVE BOUNDARY OUTPUT [DATES]')
  const result = await readLuzernTimetable(archive, boundary, dateList.split(','))
