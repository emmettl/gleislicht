...
describe('Solothurn Interlaken platform-group crosswalk', () => {
  it('routes every observed IC61/ICE Interlaken West–Ost pair in its full seasonal context, preserving successful paths', () => {
    const directions = new Set(), platforms = new Set(); let checked = 0
    for (const d of context.snapshots) for (const t of d.trains) {
      ...
      for (let i = 0; i < before.length; i++) {
        ...
        directions.add(`${a[2]}→${b[2]}`); platforms.add(a[2] === 'Interlaken Ost' ? a[3] : b[3]); checked++
      }
    }
    expect(checked).toBeGreaterThan(20); expect(directions.size).toBe(2); expect([...platforms].sort()).toEqual(['5', '7'])
  })
