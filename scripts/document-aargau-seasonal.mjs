...
const witnessGeometry = await readJson('data/aargau-witnesses/lenzburg-review-summary.json')
assert.equal(witnessGeometry.policySha256, await hashFile('data/aargau-witness-lenzburg-policy.json'))
assert.equal(witnessGeometry.previousReviewSha256, await hashFile('data/aargau-witnesses/schupfart-review-summary.json'))
...
const release = { schemaVersion: 1, annualWitnessLenzburgReviewSha256: await hashFile('data/aargau-witnesses/lenzburg-review-summary.json'), annualWitnessLenzburgPolicySha256: await hashFile('data/aargau-witness-lenzburg-policy.json'), annualWitnessSchupfartReviewSha256: await hashFile('data/aargau-witnesses/schupfart-review-summary.json'), annualWitnessSchupfartPolicySha256: await hashFile('data/aargau-witness-schupfart-policy.json'), annualWitnessOberentfeldenDetourSha256: await hashFile('data/aargau-witnesses/oberentfelden-detour-review.json'), annualWitnessOberentfeldenReviewSha256: await hashFile('data/aargau-witnesses/oberentfelden-review-summary.json'), annualWitnessOberentfeldenPolicySha256: await hashFile('data/aargau-witness-oberentfelden-policy.json'), annualWitnessBusInventorySha256: await hashFile('data/aargau-witnesses/bus-inventory.json'), annualWitnessAvaReviewSha256: await hashFile('data/aargau-witnesses/ava-review-summary.json'), annualWitnessAvaPolicySha256: await hashFile('data/aargau-witness-ava-policy.json'), annualWitnessEdgeReviewSha256: await hashFile('data/aargau-witnesses/edge-review-summary.json'), annualWitnessEdgePolicySha256: await hashFile('data/aargau-witness-edge-policy.json'), annualWitnessInventorySha256: await hashFile('data/aargau-witnesses/inventory.json'), annualWitnessGeometrySha256: await hashFile('data/aargau-witnesses/geometry-summary.json'), annualWitnessRailReviewSha256: await hashFile('data/aargau-witnesses/rail-review-summary.json'), annualWitnessRailPolicySha256: await hashFile('data/aargau-witness-rail-policy.json'), annualWitnessInterlakenReviewSha256: await hashFile('data/aargau-witnesses/interlaken-review-summary.json'), annualWitnessInterlakenPolicySha256: await hashFile('data/aargau-witness-interlaken-policy.json'), witnessedPreviouslyUnsampledRoutes: witnesses.witnessedRoutes, witnessDates: witnesses.selectedDates.map(s => s.date), unresolvedWitnessTemplateOccurrences: witnessGeometry.missingOccurrences, line344ReviewSha256: await hashFile(`${root}/344-alignment-followup.json`), addedSimplonOccurrences, simplonPolicySha256: await hashFile('data/aargau-simplon-policy.json'), addedScopedGapOccurrences, scopedGapOccurrencesByKind, seasonalGapPolicySha256: await hashFile('data/aargau-seasonal-gap-policy.json'), correctionPolicySha256: await hashFile('data/aargau-alignment-policy.json'), correctionRegressionSha256: await hashFile(`${root}/alignment-correction-regression.json`),
  ...
  remainingWork: [
    ...
    'Resolve the remaining witness geometry: 2,108 bus template occurrences across 73 incomplete patterns, including AVA timing/diversion, Schupfart zero-second and Lenzburg stop-distance holds; then extract and independently validate full civil days before extending the release scope.',
...
The [complete seasonal inventory](../data/aargau-seasonal/input/inventory.json) retains all 5,142 national routes, their archived canton membership and all twelve daily statuses. The [summary](../data/aargau-seasonal/summary.json) lists all 289 canton-calling route records, their geometry counts and the 45 still-inactive records. Inactivity in this sample is not discontinuation or an exclusion from the archived canton census. The [annual witness audit](AARGAU-ANNUAL-WITNESSES.md) now finds active canton-calling journeys for all 45 routes, covered by 19 selected civil dates. It independently verifies 2,019 archived trip templates and 13,007 complete calls across all 364 feed dates. Those templates introduce 280 directed patterns; the exact-template FOT candidate added 3,616 segment occurrences, and a separately scoped Interlaken parent/track-group binding now adds another 162 while preserving all 3,628 earlier paths. A final exact-template Bern platform-50 projection and Waldshut corridor policy adds the last 11 rail occurrences, preserving all 3,790 prior paths. All 194 rail patterns now have geometry. The [bus follow-up](AARGAU-WITNESS-BUS-REVIEW.md) inventories all 86 bus patterns and admits 2,466 AVA April occurrences, preserving all 3,801 earlier paths. The subsequent municipal closure-localization policy adds 1,836 September occurrences, preserving all 6,267 earlier paths and holding both Uerkenbrücke–Engelplatz directions. The Schupfart Festival follow-up verifies all 32 event trips against 113 advertised calls and adds 195 inferred road occurrences, preserving all 8,103 earlier paths. The May Lenzburg follow-up adds 582 occurrences, preserving all 8,298 earlier paths. Current compatibility is 8,880 of 10,988 occurrences; 2,108 bus gaps remain across 73 incomplete patterns, including 161 AVA timing holds, 153 September closure-crossing occurrences, twelve Schupfart zero-second intervals and 77 Lenzburg platform-F distance failures. Thirteen bus patterns are now complete. SBB’s 22 April notice supports the May closure corridors; complete calls and calendars remain GTFS evidence, and the September OSM graph postdates these services. A directed diagnostic finds the signed detour core alone needs at least 132.86 / 135.79 seconds at the historical OSM speed tags, exceeding all 120-second source intervals even with optimistic omissions; both directions retain a timing-reconciliation hold. The linked bus audit records the municipal notice and map, exact historical OSM closure junctions, 20 m exclusion buffer, source dates and attribution. These are separate audit counts, not additional complete regional feeds. The bus follow-up was checked on **9 September 2026**; the original twelve-date baseline date above is retained.
