...
const witnessGeometry = await readJson('data/aargau-witnesses/interlaken-review-summary.json')
assert.equal(witnessGeometry.policySha256, await hashFile('data/aargau-witness-interlaken-policy.json'))
assert.equal(witnessGeometry.previousReviewSha256, await hashFile('data/aargau-witnesses/rail-review-summary.json'))
...
const release = { schemaVersion: 1, annualWitnessInventorySha256: await hashFile('data/aargau-witnesses/inventory.json'), annualWitnessGeometrySha256: await hashFile('data/aargau-witnesses/geometry-summary.json'), annualWitnessRailReviewSha256: await hashFile('data/aargau-witnesses/rail-review-summary.json'), annualWitnessRailPolicySha256: await hashFile('data/aargau-witness-rail-policy.json'), annualWitnessInterlakenReviewSha256: await hashFile('data/aargau-witnesses/interlaken-review-summary.json'), annualWitnessInterlakenPolicySha256: await hashFile('data/aargau-witness-interlaken-policy.json'), witnessedPreviouslyUnsampledRoutes: witnesses.witnessedRoutes, witnessDates: witnesses.selectedDates.map(s => s.date), unresolvedWitnessTemplateOccurrences: witnessGeometry.missingOccurrences, line344ReviewSha256: await hashFile(`${root}/344-alignment-followup.json`), addedSimplonOccurrences, simplonPolicySha256: await hashFile('data/aargau-simplon-policy.json'), addedScopedGapOccurrences, scopedGapOccurrencesByKind, seasonalGapPolicySha256: await hashFile('data/aargau-seasonal-gap-policy.json'), correctionPolicySha256: await hashFile('data/aargau-alignment-policy.json'), correctionRegressionSha256: await hashFile(`${root}/alignment-correction-regression.json`),
  ...
  remainingWork: [
    ...
    'Resolve the remaining witness geometry: 11 rail template occurrences at Waldshut and Bern, and 7,187 bus template occurrences; then extract and independently validate full civil days before extending the release scope.',
