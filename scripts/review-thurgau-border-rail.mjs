...
const permissive = thurgauBorderGraph(JSON.parse(gunzipSync(await readFile('data/thurgau-border-rail-sources/osm.json.gz'))), { ...matcher.policy, limits: { ...matcher.policy.limits, maximumTurnDegrees: 180 } })
...
for (const platform of ['2', '3']) for (const reverse of [false, true]) {
  ...
  rows.push({ platform, reverse, strict: matcher.match(...pair), permissive: permissive.match(...pair) })
}
