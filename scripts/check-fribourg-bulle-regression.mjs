...
import { BULLE_PAIRS } from './fribourg-bulle.mjs'
import { execFileSync } from 'node:child_process'
import { readFile, writeFile } from 'node:fs/promises'

const baseline = '52a1ae7'
...
for (const date of ['2026-09-04', '2026-09-06']) {
  ...
  assert(added.every(t => t.roadReviewKinds?.includes('bulle-directed-platform-exit')), 'New journey outside Bulle evidence')
  assert.equal(added.length, date === '2026-09-04' ? 26 : 14)
  assert.deepEqual([...new Set(added.map(t => t.directionId))].sort(), ['0'])
  assert.deepEqual(['92-258-j26-1', '92-454-j26-1'].map(r => added.filter(t => t.routeId === r).length), date === '2026-09-04' ? [9, 17] : [4, 10])
  for (const t of added) {
    assert(['92-258-j26-1', '92-454-j26-1'].includes(t.routeId))
    const stopIds = t.stops.map(([i]) => b.stops[i][4])
    const matches = BULLE_PAIRS.flatMap((pair, p) => stopIds.flatMap((id, i) => id === pair[0] && stopIds[i + 1] === pair[1] ? [{ p, i }] : []))
    assert.equal(matches.length, 1)
    const { p, i } = matches[0]
    assert.equal(t.directionId, '0')
    assert.equal(t.routeId, p === 0 ? '92-258-j26-1' : '92-454-j26-1')
    assert.equal(t.stops[i + 1][1] - t.stops[i][2], 360, 'Changed original six-minute Bulle–Vuadens interval')
  }
  ...
  days.push({ date, previousJourneys: previous.size, presentJourneys: present.size, addedJourneys: added.length,
    ...
    newJourneys: added.map(t => ({ id: t.id, routeId: t.routeId, sourceServiceDate: t.sourceServiceDate, directionId: t.directionId,
      reviewedCalls: t.stops.filter(([i]) => BULLE_PAIRS.flat().includes(b.stops[i][4])).map(([i, ...times]) => [b.stops[i], ...times]), reviewKinds: t.roadReviewKinds })) })
}
const summary = await current('data/fribourg-audit/summary.json')
await writeFile('data/fribourg-audit/bulle-regression.json', JSON.stringify({ baselineCommit: execFileSync('git', ['rev-parse', baseline]).toString().trim(), sourceHashes: summary.sourceHashes,
  assertions: 'Every previously admitted journey retains its original stop identity, coordinates, call permissions, times, direction and segment geometry. Added journeys must carry reviewed Bulle road evidence.', days }, null, 2) + '\n')
