...
const busRoadTrips = day => { const pairs = new Map(day.directedStopPairs.map(p => [p.key, p])); return day.directedPatterns.filter(p => p.admitted && p.mode === 'bus' && p.pairKeys.some(k => pairs.get(k).geometrySource.startsWith('osm-'))).reduce((n, p) => n + p.trips, 0) }
...
const measures = [
  ...
  ['Admitted journeys using reusable OSM road pairs', d => d.admittedTripsUsingRoads],
  ...
  ['Journeys using strict access-road supplement (may overlap other road rows)', d => d.admittedTripsUsingAccessRoads],
  ['Directed pairs using strict access-road supplement', d => d.accessRoadDirectedPairs],
  ['Journeys using any OSM bus source, without double counting', busRoadTrips],
  ...
  ['Admitted journeys using SBB Konstanz border curves (overlap with federal rail row)', d => d.admittedTripsUsingBorderRail],
  ['Directed pairs using SBB border curves', d => d.borderRailDirectedPairs],
  ['Admitted complete lake journeys', d => d.admittedBoatTrips],
  ['Matched directed lake pairs (including excluded full journeys)', d => d.boatDirectedPairs],
...
const roadRouteIds = new Set(audit.days.flatMap(d => d.directedStopPairs.filter(p => ['osm-road-inference', 'osm-road-pattern-inference', 'osm-access-road-inference'].includes(p.geometrySource) && p.admittedOccurrences).map(p => p.routeId)))
...
const borderRouteIds = new Set(audit.days.flatMap(d => d.directedStopPairs.filter(p => p.geometrySource === 'sbb-border-rail-inference' && p.admittedOccurrences).map(p => p.routeId)))
const boatRouteIds = new Set(audit.days.flatMap(d => d.directedStopPairs.filter(p => p.geometrySource === 'swisstopo-boat-inference' && p.admittedOccurrences).map(p => p.routeId)))
const osmBoatRouteIds = new Set(audit.days.flatMap(d => d.directedStopPairs.filter(p => p.geometrySource === 'osm-boat-pattern-inference' && p.admittedOccurrences).map(p => p.routeId)))
...
const routeRows = audit.inventory.map(r => row([`\`${r.routeId}\``, `${r.agencyId} · ${r.line}`, r.mode, r.annualTripRecords, ...r.days.map(status), [...r.sourceFeatures, ...(roadRouteIds.has(r.routeId) ? ['OSM fallback'] : []), ...(railRouteIds.has(r.routeId) ? ['FOT rail'] : []), ...(borderRouteIds.has(r.routeId) ? ['SBB border rail'] : []), ...(boatRouteIds.has(r.routeId) ? ['swissTLMRegio shipping'] : []), ...(osmBoatRouteIds.has(r.routeId) ? ['reviewed OSM ferry segment'] : []), ...(cableRouteIds.has(r.routeId) ? ['FOT cableway'] : [])].join(', ') || '—', [...new Set(r.days.flatMap(d => d.reasons))].join(', ') || '—']))
...
| SBB graphical rail, 59 returned records / 2 used | Catalogue modified 29 July 2026; data processed 2 September 2026; retrieved 8 September 2026; no individual survey date | \`${audit.borderRail.policy.sourceSha256}\` (catalogue) |
| swissTLMRegio shipping, 79 regional records / 65 lake candidates | Geometry retrieved 9 September 2026; collection updated 25 June 2026; no feature survey date | \`${audit.boats.policy.sourceSha256}\` (catalogue) |
| OSM Greppen–Meggen ferry and complete SGV pattern | Historical cutoff 2 September 2026; way edited 30 January 2025; relation edited 17 July 2025; retrieved 9 September 2026 | \`${audit.boats.osm.policy.files.find(f => f.file === 'ferries.json.gz').rawSha256}\` (raw response) |
| FOEN Vector25 lake polygons, full parts and rings | Data status 1 January 2007; geometry retrieved 9 September 2026 | \`${audit.boats.source.files.find(f => f.file === 'lakes.json.gz').rawSha256}\` (raw response) |
...
The final four bus exclusions are now resolved by a separately retained **strict access-road supplement**: one Friday 101 journey via Baldegg Kantonsschule, two Friday 233 journeys via Heiligkreuz Witebach, and one Sunday EV3 journey via Entlebuch Bahnhof. The [adapter](../scripts/luzern-access-roads.mjs) prepares all **19 complete fixture patterns** on those three exact route/operator identities. Its reviewed configuration adds service roads to the OSM keep list and reduces both station-candidate and edge-snap search distances to **20 m**. The original candidate locations were 142–194 m from the GTFS stops. The new run has **zero rejected segments**, with a measured maximum platform attachment of **15.1 m** across all 19 patterns. No exported 120 m attachment or detour threshold was relaxed.

The supplemental matcher’s **93** reusable pair candidates include **92** consensus successes and one context-dependent rejection. Only **eight explicitly reviewed directed pairs** are used, each pinned to its geometry hash and consulted after both cantonal geometry and the original OSM fallback fail. These pairs cover the four previously excluded journeys; all other existing paths retain precedence. Per-journey geometrySources identifies **osm-access-road-inference**, and every affected pair retains the original official and road rejection evidence. [Regression digests](../data/luzern-access-road-regression.json), anchored to 16bd042, prove that every earlier matched path and all 12,516 / 10,397 earlier admitted journeys remain intact. [Local review panels](luzern-access-road-review.svg) show the school approach, Witebach’s two directions and the Entlebuch station loop.

The [retained source](../data/luzern-access-road-sources/source.json) is **Geofabrik Switzerland 2026-09-02**, retrieved 8 September 2026. The national OSM graph filtered by type/tag is retained as compressed XML, along with both matcher configurations and the exact filtering GTFS input; it is not claimed to be a spatial crop. Source PBF, filtered graph, configurations, full-pattern inputs and matcher outputs have separate SHA-256 evidence. Matching uses pfaedle commit **99f2cd466696ecc6bdb73b2b3bb9008557fcb84a**; the upstream configuration license and attribution are retained. The inferred paths do not certify bus access permissions, one-way street compliance or temporary diversions. Complete bus fixture coverage does not establish complete cantonal transport or year-round coverage.
...
All **45 complete boat patterns** and **44 original GTFS dock records** are retained in [offline inputs](../data/luzern-boat-inputs.json). The [policy](../data/luzern-boat-policy.json) fixes all eight route/operator/type identities, every original dock ID, and the exact source feature set for each lake. Source curves connect only at shared vertices at seven-decimal coordinate precision; no topology bridge or invented water path is added. Each directed pair is matched in every complete pattern context, and reuse requires the same accepted path in all of them. Reverse service is independently matched. The consensus inventory contains **115 route-specific directed pairs: 91 accepted and 24 rejected by the official-source matcher; the reviewed OSM supplement below resolves one rejection**. Successful pairs on an excluded whole journey remain measured in the denominator but are not exported as partial trips.

Dock attachment is capped at **150 m**; total path length including attachments must be ≤max(1,200 m, 3 × direct distance). Every path edge is split at every original shoreline or island-ring intersection, so a thin island cannot disappear between sample points. Any outside-water interval rejects the complete pattern unless the entire interval lies within **one actual endpoint dock’s 150 m zone**. Such intervals remain explicitly recorded as discrepancies between generalized shipping geometry, the GTFS dock and the 2007 shoreline. They are not claims that vessels travel on land. The maximum attachment among delivered boat pairs is **97.2 m**. Delivered pairs contain **66 Friday / 103 Sunday dock-zone outside-water intervals** (pair records, not multiplied by journeys), with maximum interval lengths **26.0 / 48.6 m**. [Six review panels](luzern-boat-review.svg) show both Hallwilersee circuits, the Bürgenstock shuttle, the full Meggenhorn journey, the rejected official Greppen–Meggen path alongside its admitted OSM replacement and a dock discrepancy at Seengen.

The supplement adds **${friday.admittedBoatTrips} Friday / ${sunday.admittedBoatTrips} Sunday complete lake journeys**: **41 / 45 SGV**, and **3 / 9 Hallwilersee**. Every Hallwilersee fixture journey is admitted, including its repeated first/last dock and different Sunday circuit. SGV 3602 is complete at **33/33 each day**, 3604 at **6/6 each day**, and 3603 at **2/2 Friday and 6/6 Sunday**. Overall lake coverage is **44/86 Friday and 54/104 Sunday**, across **7 and 11 complete admitted boat patterns**. The feeds use the existing ferry display category, preserve all stops, times and call rules, and identify each segment as **swisstopo-boat-inference** or **osm-boat-pattern-inference**. [Regression digests](../data/luzern-boat-regression.json), anchored to **7c977ea**, preserve every earlier matched path and all **12,541 / 10,424** earlier admitted journeys.

A narrowly scoped [OSM supplement](../scripts/luzern-osm-boats.mjs) resolves **Greppen → Meggen** for the complete Sunday pattern **a54c8947555d19d0ae31**: Küssnacht → Greppen → Meggen → Verkehrshaus-Lido → Luzern Bahnhofquai **dock 3**. The full ordered SGV relation **18628404** has matching ref **3603**, operator **SGV**, four matching UIC dock identities and an explicit fifth platform binding: OSM node **6686811677**, name “Luzern Bahnhofquai 3”, local_ref **3**, to GTFS **ch:1:sloid:8492:0:929446**, platform **3**. OSM supplies no UIC on that node; the reviewed alias is recorded rather than inventing a source tag. Verkehrshaus’s approximately 90 m identity comparison does not authorize geometry attachment there: its previously accepted official segment is preserved.

Only original way **1355336513**, with **11 source vertices**, supplies replacement geometry. All ways of the complete relation are traversed in source-member order using actual shared node IDs; stop order, operator, line and original platform are binding. The replaced pair occurs in exactly one complete fixture pattern; any changed call rules, direction or additional complete context fails the review. Its original GTFS endpoint attachments are **23.714 m / 29.689 m**, within a separate **30 m** limit. The resulting **3,899.54 m** path takes the original **900 s** interval (mean **15.60 km/h**, below the 25 km/h review limit). Full shoreline intersection checks find one **11.84 m** outside-water interval wholly within Meggen’s existing 150 m dock zone. The remote land crossing of the original generalized curve remains in primaryAssessment. All four segments, five original calls and original times of the newly admitted journey survive; no Friday journey is added and no successful earlier geometry is replaced. [Regression digests](../data/luzern-osm-boat-regression.json), anchored to **3d98c67**, preserve every earlier matched path and all **12,585 / 10,477** previously admitted journeys.

The [retained OSM catalogue and review policy](../data/luzern-osm-boat-policy.json) pins the complete recursive historical Overpass response: **1,303 elements**, including **56 ferry relations, 95 ways and 66 ferry-terminal nodes** (the remaining nodes are original geometry vertices). The query covers **46.85–47.12° N, 8.25–8.65° E**, with complete recursively referenced members, and is retained verbatim with compressed/raw hashes. This is an envelope census, not a nationwide OSM completeness claim. Historical cutoff is **2026-09-02 00:00 UTC**, retrieved **9 September 2026**; the selected way is version **1**, last edited **30 January 2025**, and the selected relation is version **3**, last edited **17 July 2025**. Edit timestamps are not survey dates. All original source metadata remains retained. Geometry attribution is **[© OpenStreetMap contributors](https://www.openstreetmap.org/copyright)** under **[ODbL 1.0](https://opendatacommons.org/licenses/odbl/1-0/)**; shoreline attribution remains **© FOEN, swisstopo**. Feed metadata identifies the policy, source dates and licence, and the machine audit inventories every relation, way and terminal, including all unused records.

The remaining **42 Friday / 50 Sunday SGV journeys** remain excluded in full, affecting routes **3600, 3601 and 3605**. Some Luzern Bahnhofquai dock IDs attach **221.8–227.2 m** away from official linework, Hergiswil **190.4 m**, and Alpnachstad **350.9 m**, exceeding the unchanged 150 m official-source limit. Matching preserves separate dock IDs. OSM 3600 relations use dock **4**, which does not authorize the excluded GTFS dock **1**; relation **18623434** has conflicting operator “Verkehrshaus”, and **18615633** lacks an operator. The 3601 records remain retained for further full-pattern review, including relation **18629129** without an operator. Relation **18624219** names Saphir but has ref **3604** and calls **Schweizerhofquai**, so it does not establish the GTFS 3605 Bahnhofquai circuit. No route substitution or dock alias is inferred from those records. Other returned relations, including Zug, Ägeri and Beckenried–Gersau, remain source-only evidence with no geometry admission.
...
Review regenerated cache/evidence hashes before updating policy. Matcher elapsed times and warning-log timings can change between runs; the committed evidence preserves the measured run. No changed cache can silently replace the pinned input. All **75 scoped unit tests pass**, including strict access-road pair selection, failed-context rejection and changed-source/identity isolation, consensus failure/conflict isolation, explicit full-pattern exception isolation and pinned geometry, repeated-pair loops, reversed directions, changed identities, corrupt indices/endpoints, source hashes, detour/collapse limits and routing-only carry-in normalization. Rail tests additionally cover exact/ambiguous operating-point identities, reversed source geometry, called-station order, conflicting complete patterns, gauge/validity exclusion, station/topology attachment limits and detour rejection. Cableway tests cover source vertices, reversal, exact station/installation/operator identity, explicit aliases, station limits, source validity, disconnected geometry and malformed coordinates.

To reproduce the strict access-road run, decompress data/luzern-access-road-sources/network.osm.gz to a temporary .osm file; prepare all three route feeds with scripts/luzern-access-roads.mjs using the pinned timetable; run scripts/match-postbus-roads.mjs separately for agencies 801, 839 and 7231 using the retained routing.cfg and matcher binary. Import with importLuzernRoads from scripts/luzern-road-geometry.mjs, passing the retained source.json as its fifth argument. The normal regional checker independently rebuilds the cache from all retained matcher output, validates the complete 19-pattern scope on both dates, rechecks the eight review hashes and compares every delivered path. Regenerating an input does not authorize replacing its pinned hash.
