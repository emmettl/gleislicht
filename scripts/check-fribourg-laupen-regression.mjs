...
import { LAUPEN_PAIRS } from './fribourg-laupen.mjs'
import { execFileSync } from 'node:child_process'
import { readFile, writeFile } from 'node:fs/promises'

const baseline = '71f8a5e'
...
for (const date of ['2026-09-04', '2026-09-06']) {
  ...
  assert(added.every(t => t.roadReviewKinds?.includes('laupen-western-bypass')), 'New journey outside Laupen evidence')
  assert.equal(added.length, date === '2026-09-04' ? 59 : 20)
  for (const t of added) {
    assert.equal(t.routeId, '96-881-j26-1')
    const stopIds = t.stops.map(([i]) => b.stops[i][4])
    const matches = LAUPEN_PAIRS.flatMap((pair, p) => stopIds.flatMap((id, i) => id === pair[0] && stopIds[i + 1] === pair[1] ? [{ p, i }] : []))
    assert.equal(matches.length, 1)
    const { p, i } = matches[0]
    assert.equal(t.directionId, p === 2 ? '1' : '0')
    assert(p === 2 ? i === 0 : i === t.stops.length - 2)
    assert([240, 300].includes(t.stops[i + 1][1] - t.stops[i][2]), 'Changed original four/five-minute diversion interval')
  }
  ...
  days.push({ date, previousJourneys: previous.size, presentJourneys: present.size, addedJourneys: added.length,
    ...
    newJourneys: added.map(t => ({ id: t.id, routeId: t.routeId, sourceServiceDate: t.sourceServiceDate, directionId: t.directionId,
      reviewedCalls: t.stops.filter(([i]) => LAUPEN_PAIRS.flat().includes(b.stops[i][4])).map(([i, ...times]) => [b.stops[i], ...times]), reviewKinds: t.roadReviewKinds })) })
}
const summary = await current('data/fribourg-audit/summary.json')
await writeFile('data/fribourg-audit/laupen-regression.json', JSON.stringify({ baselineCommit: execFileSync('git', ['rev-parse', baseline]).toString().trim(), sourceHashes: summary.sourceHashes,
  assertions: 'Every previously admitted journey retains its original stop identity, coordinates, call permissions, times, direction and segment geometry. Added journeys must carry reviewed Laupen road evidence.', days }, null, 2) + '\n')
