...
const panels = [
  ...
  { pairNames: ['Greppen', 'Meggen (See)'], title: 'Greppen–Meggen · official rejection / OSM admission', comparison: true },
...
for (const [i, panel] of panels.entries()) {
  ...
  if (panel.pattern) {
    ...
    if (panel.comparison) {
      const path = matchBaselSegment(boats.lakes.get(r.lake).graph, ...ids.map(coord), boats.policy.limits).path
      assert.equal(sha256(JSON.stringify(path)), r.primaryAssessment.rejectedGeometrySha256)
      paths = [path, r.path]; results = [r.primaryAssessment, r]
    } else if (r.path) paths = [r.path]
  ...
  for (const [j, path] of paths.entries()) svg += line(path, panel.comparison && j === 0 ? '#c33f47' : '#008580')
  ...
  svg += '</g>'
  const note = panel.comparison ? 'Full Sunday journey admitted · 11 OSM vertices · 23.7 / 29.7 m dock attachments' : panel.detail ? 'Outside-water intervals are allowed only wholly within one 150 m dock zone' : `${ids.length} original calls · complete pattern retained · maximum dock attachment ${Math.max(...results.map(r => r.maximumSnapMetres)).toFixed(1)} m`
...
svg += '<text x="25" y="1450" class="small">Shipping: © swisstopo / © OpenStreetMap contributors (ODbL-1.0) · shoreline: © FOEN, swisstopo · timetable: SBB / opentransportdata.swiss</text><text x="25" y="1475" class="small">Geometry retrieved 2026-09-09 · source API gives no individual feature survey date · all shoreline rings retained</text><text x="25" y="1498" class="small">Dock discrepancies are explicit cartographic limitations, not evidence that boats travel over land.</text></svg>\n'
