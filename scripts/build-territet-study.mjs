...
export const selectTerritetRoute=r=>r.route_id==='93-TG-j26-1' && r.agency_id==='131' && r.route_type==='1400'
export { auditGornergratGeometry as auditTerritetGeometry } from './build-gornergrat-study.mjs'
import { auditGornergratGeometry as auditTerritetGeometry } from './build-gornergrat-study.mjs'
async function main(){
 const archive=arg('archive'),railSource=arg('rail-source'),date=arg('date','2026-09-04')
 if(!archive || !railSource)throw new Error('Use --archive GTFS.zip --rail-source territet-fot.json')
 ...
 for await(const r of rowsFromArchive(archive,'routes.txt'))if(selectTerritetRoute(r))routes.set(r.route_id,r)
 if(routes.size!==1)throw new Error('Expected the exact Territet funicular route')
 for await(const t of rowsFromArchive(archive,'trips.txt'))if(routes.has(t.route_id) && services.has(t.service_id))trips.set(t.trip_id,{route:'TG',routeId:t.route_id,agencyId:'131',operator:operators.get('131'),category:'funicular',mode:'funicular',headsign:t.trip_headsign,shortName:t.trip_short_name})
 ...
 const builder=createSnapshotBuilder({trips,sourceStops,windowStart:0,windowEnd:86400,focusTime:43440,displayBounds:{minLongitude:-180,maxLongitude:180,minLatitude:-90,maxLatitude:90}})
 await readStopTimes(archive,trips,[builder],frequencies,0,86400)
 const snapshot=builder.finish();snapshot.trains=snapshot.trains.map(t=>({...t,routeId:'93-TG-j26-1',agencyId:'131',operator:operators.get('131'),routeType:1400}))
 const {alignmentReview,...rail}=applyTerritetGeometry(snapshot,JSON.parse(xml))
 ...
 const metadata={publisher:feed.feed_publisher_name,feedVersion:feed.feed_version,serviceDate:date,windowStart:0,windowEnd:86400,focusTime:43440,sourceUrl:'https://opentransportdata.swiss/en/cookbook/timetable-cookbook/gtfs/',model:'Scheduled interpolation on mapped 2D FOT funicular alignment; no live positions or measured terrain.',note:'Exact Territet–Glion funicular route only; GTFS type 1400. Original service day; no preceding-day spillover or seasonal comparison.',sources:{timetable:{sha256:await checksum(archive)},rail:{publisher:'Federal Office of Transport',sha256:await checksum(railSource),simplificationToleranceMetres:0}}}
 const result={...snapshot,...rail,metadata};delete result.audit
 const geometry=auditTerritetGeometry(result),counts={trips:result.trains.length,stops:result.stops.length,paths:result.paths.length,gzipBytes:gzipSync(JSON.stringify(result)).length}
 ...
 await writeFile(arg('audit-output','data/territet-study-audit.json'),JSON.stringify({metadata,counts,alignmentReview,geometry},null,2)+'\n')
 if(!geometry.passed || counts.gzipBytes>40*1024)throw new Error('Territet geometry or payload gate failed')
 await writeFile(arg('source-output','data/territet-journey-source.json'),JSON.stringify(evidence)+'\n')
 await writeFile(arg('output','public/data/territet-day.json'),JSON.stringify(result)+'\n')
