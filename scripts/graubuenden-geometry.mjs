...
export async function loadGraubuendenGeometry(policy, raw) {
  ...
  return { railInventory, roads, matchPattern(train, route) {
    if (route.mode === 'rail') return rails.has(route.routeId) ? rails.get(route.routeId).matchPattern(train, stops, route) : train.calls.slice(1).map(() => ({ reason: 'unreviewed-rail-identity' }))
