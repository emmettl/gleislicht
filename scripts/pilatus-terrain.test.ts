...
import terrain from '../public/data/pilatus-ascent-terrain.json'
import audit from '../data/pilatus-terrain-audit.json'
import {pilatusJourneys} from '../src/studies/pilatus.ts'
import {bindPilatusTerrain} from '../src/studies/pilatus-terrain.ts'
...
const up=pilatusJourneys(network,'ascent'),down=pilatusJourneys(network,'descent')
describe('Pilatus measured terrain binding',()=>{
 it('preserves every source call and rail height in both directions',()=>{
  expect(up).toHaveLength(17);expect(down).toHaveLength(17)
  for(const train of [...up,...down]){
   const binding=bindPilatusTerrain(terrain,network,train)!
 ...
 it('reverses audited positions and structure masks, keeping every masked interval out of 3D',()=>{
  for(const train of [...up, ...down]){
   const binding=bindPilatusTerrain(terrain,network,train)!
   expect(binding.windows).toHaveLength(4)
 ...
 it('requires explicit audited train IDs and valid dated source data',()=>{
  for(const train of [up[0],down[0]]){
   ...
   expect(bindPilatusTerrain(missing,network,train)).toBeUndefined()
   ...
   for(const data of bad)expect(bindPilatusTerrain(data,network,train)).toBeUndefined()
  }
 })
 it('pins source hashes and keeps the optional 36 m mesh below 120 KiB',()=>{
  const hash=(path:string)=>createHash('sha256').update(readFileSync(path)).digest('hex')
  expect(terrain.metadata.networkSha256).toBe(hash('public/data/pilatus-day.json'))
  expect(terrain.metadata.railSourceSha256).toBe(hash('data/pilatus-terrain-source.json'))
  expect(gzipSync(readFileSync('public/data/pilatus-ascent-terrain.json')).length).toBeLessThan(120*1024)
  expect(audit.routes[0].maxOffsetMetres).toBeLessThan(13)
  expect(audit.routes[0].stops.at(-1)!.railHeight).toBeCloseTo(2059.631,2)
  expect(terrain.metadata.gridSpacingMetres.every(n=>n>36 && n<37)).toBe(true)
 })
})
