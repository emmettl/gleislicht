...
const policy = await json('data/fribourg-policy.json')
const rail = await loadFribourgRail(undefined, policy.rail)
...
const panels = ['S20', 'S21', 'RE2', 'IC1'].map((line, i) => {
  const route = routes.find(r => r.agencyId === (line === 'IC1' ? '11' : '53') && r.name === line)
  ...
  const pairs = report.directedPairs.filter(p => p.routeId === route.id && p.geometrySource === 'fot-rail-inference')
  const inferred = pairs.map(p => rail.pairs.get(JSON.stringify([p.routeId, p.fromId, p.toId])).path)
  ...
  return `<g transform="translate(${20 + (i % 2) * 590},${98 + Math.floor(i / 2) * 465})"><rect width="570" height="445" rx="10" fill="white" stroke="#cbd5df"/><text x="24" y="32" font-size="21" font-weight="bold">${line === 'IC1' ? 'SBB' : 'TPF'} ${line}</text><text x="158" y="31" font-size="14">Friday: ${day.admittedTrips}/${day.trips} journeys admitted</text>${draw(official, '#9aaaba', 2.4)}${draw(inferred, '#e97924', 1.9)}${markers}<text x="24" y="426" font-size="13">${pairs.length} inferred directed pairs; ${gap.length} unresolved directed pairs</text></g>`
})
const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="1220" height="1060" viewBox="0 0 1220 1060"><rect width="1220" height="1060" fill="#edf2f6"/><g font-family="Arial, sans-serif" fill="#203246"><text x="24" y="35" font-size="25" font-weight="bold">Fribourg rail geometry review · 4 September 2026</text><text x="24" y="61" font-size="15">Grey: cantonal source lines. Orange: accepted FOT/SBB rail fallback, including pairs in excluded journeys.</text><text x="24" y="82" font-size="13">North up · separate panel scales · inferred paths and bounded platform connectors; physical direction is not certified.</text>${panels.join('')}<text x="24" y="1042" font-size="12">Source: Etat de Fribourg · © Federal Office of Transport (FOT) · SBB Infrastructure / data.sbb.ch · FOT catalogue 2021-07-06, 2026 alignment validity unknown · timetable: opentransportdata.swiss</text></g></svg>`
await mkdir('docs/assets', { recursive: true })
await writeFile('docs/assets/fribourg-rail-review.svg', svg)
console.log('Generated four rail corridor review panels')
