...
${table(['Direction / source platform','Full path','Track attachments','Source interval / mean speed','Trips Friday / Sunday'],audit.comoRailPairs.map((p,i)=>[i===0?'Chiasso platform 1 → Como':'Como → Chiasso platform 6',(p.pathMetres/1000).toFixed(3)+' km',p.trackAttachmentsMetres.map(m=>m.toFixed(2)+' m').join(' / '),audit.comoRailTiming[0].pairs[i].intervals[0].seconds/60+' min / '+audit.comoRailTiming[0].pairs[i].meanKmh[0].toFixed(2)+' km/h',audit.comoRailTiming.map(d=>d.pairs[i].intervals.length).join(' / ')]))}

The independent [Lombardia mobility layer](https://www.cartografia.servizirl.it/expo/rest/services/gpt/infrastrutture_mobilita/MapServer/3) supplies **eight** complete corridor-envelope rail records, reconciled against a separate object-ID query. Five records describe the Chiasso–Como approach/tunnel corridor; all their original vertices lie within **${Math.max(...audit.comoComparisonInventory.filter(f=>f.use==='independent-corridor-review-only').map(f=>f.maximumVertexGapMetres)).toFixed(2)} m** of the OSM southbound path. This is a vertex-to-polyline comparison, not a continuous Hausdorff bound or running-track certification. The other three records cover the Como Lago branch, onward southern line and longer tunnel bypass. All eight records, their properties and comparison distances are retained. The GeoJSON request failed; the native ArcGIS JSON request succeeded. No Lombardia coordinate is spliced into the feed.
...
${table(['Directed pair','Graph-only length','Full path including connectors','Frozen interval','Implied mean','Decision'],audit.boatReview.trials.map(t=>[t.from+' → '+t.to,(t.candidate.pathMetres/1000).toFixed(3)+' km',t.fullPathMetres===null?'no accepted candidate':(t.fullPathMetres/1000).toFixed(3)+' km',[...new Set(t.intervals.map(i=>i.seconds/60))].join(', ')+' min',t.fullPathMetres===null?'≥ '+t.intervals[0].graphMeanKmh.toFixed(2)+' km/h (graph only)':t.intervals[0].fullPathMeanKmh.toFixed(2)+' km/h',t.candidate.reason??'water-valid; service path unresolved']))}
...
${table(['Run','Matched / all occurrences','Importer maximum accepted snap','Grienbach → V-Zug length','Implied mean over 60 seconds','Admission'],audit.grienbachReview.runs.map(r=>[r.id,r.report.matchedSegments+'/'+r.report.totalSegments,r.report.maxSnapMetres.toFixed(2)+' m',r.pairs[1].lengthMetres===null?'rejected':r.pairs[1].lengthMetres.toFixed(2)+' m',r.impliedAverageKmh.length?r.impliedAverageKmh[0].values[0].toFixed(2)+' km/h':'—','excluded']))}
...
${table(['Original GTFS platform','Stop','Nearest same-UIC OSM position','Friday / Sunday calls'],audit.grienbachReview.platformReview.platforms.map(p=>[p.stopId,p.name,p.minimumSameUicDistanceMetres.toFixed(2)+' m',p.days.map(d=>d.contexts.reduce((n,c)=>n+c.trips,0)).join(' / ')]))}
...
[Review catalogue](../data/zug-road-context-sources/sources.json) records every full-pattern ID, terminus and geometry hash. The underlying geometry remains the original [expanded road cache](../data/zug-road-expansion-cache.json) and [matcher evidence](../data/zug-road-expansion-evidence/839.json.gz): Geofabrik Switzerland **2026-09-02** plus the border extract retrieved **2026-09-08**, matched September 8. A separate [bounded station-area OSM inspection](../data/zug-road-context-sources/roads.json.gz), [query](../data/zug-road-context-sources/roads.overpass), has base timestamp **${audit.roadContextSource.stationReviewOsmBase}**. It documents Bahnhofstrasse and nearby service-road topology; it does not replace the original matcher paths or establish historical operational validity. Both sources carry **© OpenStreetMap contributors**, **ODbL-1.0**; [licence and attribution](https://www.openstreetmap.org/copyright).

${table(['Full road-pattern ID','Terminus GTFS ID','Vertices','Length'],audit.roadContextInventory.flatMap(i=>i.variants.map(v=>[v.patternId,v.terminus,v.vertices,v.lengthMetres.toFixed(1)+' m'])))}
...
[Source catalogue](../data/zug-service-road-sources/sources.json), [local OSM JSON](../data/zug-service-road-sources/roads.json.gz), [routing XML](../data/zug-service-road-sources/roads.osm.gz), [query](../data/zug-service-road-sources/roads.overpass), [configuration](../data/zug-service-road-sources/pfaedle.cfg), [trial cache](../data/zug-service-road-sources/cache.json), [control cache](../data/zug-service-road-sources/control-cache.json) and both compressed matcher-evidence bundles are preserved. The bounded query includes road ways, their nodes, public-transport nodes, local restriction relations and their recursive members: **${audit.serviceRoadSource.elementCount} OSM elements**. Road source timestamp: **${audit.serviceRoadSource.osmBase}**; retrieved **${audit.serviceRoadSource.retrieved}**. This is a September 8 snapshot, not proof that every road element was identical on September 4 or 6. Credit: **© OpenStreetMap contributors**, **ODbL-1.0**, with [attribution/licence requirements](https://www.openstreetmap.org/copyright).
