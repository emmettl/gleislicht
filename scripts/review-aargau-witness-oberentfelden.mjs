...
import { OBERENTFELDEN_POLICY,loadWitnessOberentfelden } from './aargau-witness-oberentfelden.mjs'
...
const prior=await readGzipJson(`${root}/ava-review-patterns.json.gz`),priorSummary=await readJson(`${root}/ava-review-summary.json`)
const context=await loadSeasonalContext('data/aargau-seasonal/input'),rail=await loadWitnessRail(),interlaken=await loadWitnessInterlaken(),edges=await loadWitnessEdges(),ava=await loadWitnessAva(),ober=await loadWitnessOberentfelden()
assert.equal(priorSummary.policySha256,await hashFile('data/aargau-witness-ava-policy.json'))
assert.equal(priorSummary.geometryPatternsSha256,await hashFile(`${root}/ava-review-patterns.json.gz`))
...
for(const t of raw.trains){const stops=t.calls.map(c=>raw.stops.find(s=>s[4]===c[0]));rail.assertTemplate(t,stops);interlaken.assertTemplate(t,stops);edges.assertTemplate(t,stops);ava.assertTemplate(t,stops);ober.assertTemplate(t,stops)}
const rails={matchPattern:(t,stops)=>context.rails.matchPattern(t,stops)??rail.matchPattern(t,stops)}
const previousPlatforms={matchPattern:(t,stops)=>edges.matchPattern(t,stops)??ava.matchPattern(t,stops)}
...
const platforms={matchPattern:(t,stops)=>{
 const previous=previousPlatforms.matchPattern(t,stops),extra=ober.matchPattern(t,stops)
 return extra?.map((s,i)=>previous?.[i]?.path?previous[i]:s??previous?.[i])??previous
}}
...
assert.equal(regression.preservedOccurrences,priorSummary.compatibleOccurrences);assert.equal(regression.addedOccurrences,ober.policy.expected.addedOccurrences)
...
const oldById=new Map(prior.patterns.map(p=>[p.id,p])),ruleById=new Map(ober.policy.patterns.map(p=>[p.id,p])),unresolved=[],failures={},addedByDirection={}
...
for(const p of after.patterns)for(const [i,s] of p.segments.entries()){
 ...
 if(s.witnessOberentfeldenPatternId){
  assert.equal(s.witnessOberentfeldenPatternId,p.id);assert.equal(old.pathIndex,null)
...
const summary={schemaVersion:1,policySha256:await hashFile(OBERENTFELDEN_POLICY),previousReviewSha256:await hashFile(`${root}/ava-review-summary.json`),previousPatternsSha256:await hashFile(`${root}/ava-review-patterns.json.gz`),inventorySha256:await hashFile(`${root}/inventory.json`),sourceVerificationSha256:await hashFile(`${root}/source-verification.json`),geometryPatternsFile:'oberentfelden-review-patterns.json.gz',geometryPatternsSha256:await hashFile(file),
 scope:'Separate annual witness-template candidate. The dated municipal notice and map localize the September closure to an exact historical OSM corridor. Admit 1,836 existing AVA road occurrences wholly clear of its 20 m exclusion buffer; hold both Uerkenbrücke–Engelplatz directions (153 occurrences). All April decisions and every prior accepted path remain identical. Full GTFS identities, coordinates, calendars and calls are preserved. These are archived trip-template counts, not date-specific regional feeds or certified running tracks.',
 attribution:priorSummary.attribution,source:ober.policy.source,oberentfeldenDispositionOccurrences:ober.policy.expected.counts,regression:{...regression,completeSourceTemplatesAndCallsPreserved:true,allOtherAssessmentsPreserved:true,previousCandidateReplayed:true},addedByDirection,
...
assert.equal(summary.compatibleOccurrences,8103);assert.equal(summary.busMissingOccurrences,2885);assert.equal(summary.fullyCompatiblePatterns,195)
if(process.argv.includes('--check'))assert.deepEqual(await readJson(`${root}/oberentfelden-review-summary.json`),summary)
else await writeFile(`${root}/oberentfelden-review-summary.json`,JSON.stringify(summary,null,2)+'\n')
