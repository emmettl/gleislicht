...
export async function loadSolothurnSupplements(timetable, { roads = true, verifyEvidence = false } = {}) {
  ...
  for (const raw of context.snapshots) {
    ...
    for (const train of raw.trains) {
      ...
      if (route.mode === 'rail') results = s26.matchPattern(train, raw.stops, route, bernTerminal.matchPattern(train, raw.stops, route, railReview.matchPattern(train, raw.stops, route, results)))
