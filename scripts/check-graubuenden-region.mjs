...
for (const day of raw.snapshots) {
  ...
  function check(t, snapshot) {
    ...
    assert.equal(t.geometrySource, ['bus', 'mountain'].includes(pattern.mode) ? pattern.pairs[0].geometrySource : (pattern.pairs.find(p => p.geometrySource?.startsWith('fot-reviewed-'))?.geometrySource ?? 'fot'))
