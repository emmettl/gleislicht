...
for (const [title, match, routes, radius] of [
  ['La Plantaz · unresolved platform pair', /Tolochenaz, La Plantaz/, ['701'], .004],
...
for (const [i, p] of panels.entries()) {
  ...
  for (const points of p.rail ? rail : roads) if (points.some(inside)) svg += `<path d="${line(points)}" fill="none" stroke="#758099" stroke-width="3"/>`
  const sourceStops = p.rail ? manifest.stops : input.stops, paths = p.rail ? manifest.paths : cache.paths
  const pathIds = new Set(), stopIds = new Set()
  const patterns = p.rail ? mbcRail : input.patterns.filter(pattern => p.routes.includes(pattern.route))
  for (const pattern of patterns) {
    for (const id of p.rail ? pattern.pathSegments ?? [] : cache.patterns[pattern.id] ?? []) if (id != null) pathIds.add(id)
...
svg += '<text x="24" y="1730" font-size="14">© OpenStreetMap contributors · ODbL 1.0; Federal Office of Transport. Source alignment review, not operator verification.</text></svg>'
await mkdir(arg('output'), { recursive: true })
await writeFile(join(arg('output'), 'mbc-geometry-review.svg'), svg)
await writeFile(join(arg('output'), 'mbc-geometry-review.json'), JSON.stringify({ serviceDate: input.metadata.serviceDate, graphSha256: createHash('sha256').update(graphBytes).digest('hex'), railSha256: createHash('sha256').update(railBytes).digest('hex'), cacheSha256: createHash('sha256').update(await readFile(arg('cache'))).digest('hex'), largestSnaps: ranked.slice(0, 20), rejectedPatternSegments: cache.report.issues, panels }, null, 2) + '\n')
