...
import { rivieraRailCorridors } from './riviera-rail-geometry.mjs'
...
const {manifest,trains}=await readPostbusDay(manifestPath), roads=JSON.parse(await readFile(roadPath)),rail=Object.values(rivieraRailCorridors(parseRailNetworkXtf(await readFile(railPath,'utf8'),10))).flat().map(s=>s.points)
const panels=[['Vevey · MVR terminal',[6.843,46.464],.008,true],['Montreux · separate MOB and MVR tracks',[6.913,46.438],.009,true],['Rochers-de-Naye · complete cogwheel corridor',[6.945,46.442],.045,true],['Vevey · buses and replacement services',[6.84,46.468],.012],['Fontanivent · VMCV hill routes',[6.894,46.453],.010],['Montbovon · MOB approach',[7.043,46.487],.014,true]]
const escape=s=>s.replaceAll('&','&amp;').replaceAll('<','&lt;')
let svg='<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="1280"><rect width="1200" height="1280" fill="#0b1020"/><style>text{font-family:Arial;fill:#e8efff}</style><text x="24" y="32" font-size="22">Riviera geometry review · 8 September 2026</text><text x="24" y="59" font-size="14">Grey: retained OSM roads / FOT rail · cyan: matched journeys · white: GTFS platforms</text>'
for(const [i,[title,centre,radius,isRail]] of panels.entries()){
 ...
 const selected=trains.filter(t=>isRail?['regional','regional-express','interregio','intercity','international','s-bahn','other'].includes(t.category):t.category==='bus'),ids=new Set(selected.flatMap(t=>t.pathSegments).filter(id=>id!=null)),stops=new Set(selected.flatMap(t=>t.stops.map(([i])=>i)));let visible=0,labels=0
