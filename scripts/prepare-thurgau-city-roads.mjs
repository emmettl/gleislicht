...
const output = process.argv[2] ?? '/private/tmp/thurgau-city-road-feeds'
const reports = []
for (const id of ['727', '797']) {
  ...
  for (const raw of timetable.snapshots) for (const train of raw.trains) {
    if (train.agencyId !== id || train.route === 'NT' || train.reservationRequired) continue
  ...
  const metadata = { ...timetable.snapshots[0].metadata, serviceDates: timetable.snapshots.map(s => s.metadata.serviceDate),
    ...
    note: 'Routing-only union of complete city patterns. Shifted times are not a passenger timetable; NT night taxi excluded.' }
  reports.push({ agencyId: id, ...await prepareRoadFeed({ manifest: { metadata, stops }, trains, output: join(output, id),
    agency: { id, name: route.agency, url: id === '797' ? 'https://www.frauenfeld.ch' : 'https://www.kreuzlingen.ch' } }) })
}
