...
The [seasonal compatibility and release audit](AARGAU-SEASONAL-AUDIT.md) extends validation to twelve dates: **110,050 complete journeys, 1,821,846 calls and 1,146 directed patterns absent from September**. It finds 12 newly active routes and 45 still inactive in the sample. A 460-pattern road extension fills 16,364 seasonal gaps, bringing compatibility to 99.944–100% while preserving every prior path. A separate Friday review candidate corrects the line 136 Wittnau branch traversal; 223 of the original 224 flagged directed bus pairs remain under review. The archived September feeds still replay exactly; application release remains pending these reviews.
...
Thurbo announces a **14 September–2 October 2026** S36 closure on this crossing. The rule is therefore explicitly limited to **4 and 6 September**; it cannot silently apply to later seasonal fixtures or replacement buses. That scope does not independently certify the running track.
...
The source preparer deliberately rejects an AGIS archive differing from the survey's pinned hash. A refresh requires a newly reviewed catalogue/vintage and regeneration of dependent inventory/audit hashes; a mutable download URL must not silently replace this evidence. `publicationReady` remains false: the twelve-date follow-up leaves 585 sampled geometry occurrences and 223 flagged bus pairs requiring further evidence before application release. Complete timetable preservation and consistency/payload checks pass; source compatibility does not certify actual historical or future operation.
