...
**Seasonal follow-up:** The [twelve-date audit](AARGAU-SEASONAL-AUDIT.md) independently verifies 110,050 journeys and 1,821,846 calls, identifies 1,146 directed patterns absent from September and 12 newly active routes, and leaves 45 archived routes inactive in the sample. Geometry compatibility is 97.583–100%; no date-scoped policy is broadened. The September bus comparison flags 224 distinct directed route/platform pairs where AGIS and OSM differ by more than 30 m, requiring itinerary review.

**Next action:** Resolve the ranked source-alignment disagreements and seasonal geometry gaps, review date-limited platform/border exceptions, and find witness dates for the 45 unsampled routes before expanding release scope. Then integrate reviewed fixtures into the application. The complete route inventory supersedes the representative agency shortlist for Aargau membership.
