...
## Railway legs on the terrain clock

During either sequence’s railway leg, **Follow this train in terrain** switches the scene while preserving the current timetable time, selected train, pause state and playback rate. The Vitznau approach follows its whole ascent; the Weggis approach first enters at Kaltbad’s mapped fraction (about 65.7% along the full Vitznau alignment). It does not replay the earlier railway section that the visitor bypassed by cableway.

Progress interpolates between the selected train’s arrival/departure calls and the matching station fractions in the measured corridor. Dwell intervals hold the train still: for example, service 1133 remains at Kaltbad from **13:33 to 13:35**. The representative terrain service’s clock and the independent scenic loop do not control this view. The speed controls advance scheduled time; reaching the selected summit arrival pauses exactly there. Rewinding or jumping uses the new time directly.

**Return to map** preserves the clock. Scrubbing outside the boarded railway interval shows the corresponding boat, interchange or cableway map stage. With terrain following still enabled, returning to the railway interval restores terrain. The terrain and map renderers replace one another rather than running two canvases together. The existing full-day time controls remain available.

The terrain artifact is requested only when following is enabled and is reused while the sequence stays open. Its approach, timetable hash/date/version, station matches and chronology are checked before binding. A failed or incompatible load leaves the map available with retry. The optional profile identifies scheduled interpolation separately from independent scenic playback. These remain ground elevations beneath mapped rail, not surveyed track or tunnel heights; the 3D curve and vehicle are artistic interpretations. Pedestrian and vertical cable geometry remain separate work.
