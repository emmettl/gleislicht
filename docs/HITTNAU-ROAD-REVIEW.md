...
The [publisher's traffic-data page](https://www.zh.ch/de/mobilitaet/gesamtverkehrsplanung/verkehrsgrundlagen/verkehrsdaten.html) links project and operational documents in a password-protected area. No station-specific 3091 plan was obtained in the initial review. A guessed annual-report URL returned 404. The subsequent interactive-GIS check below found the actual public annual report; the earlier failed URL is not evidence of document unavailability.

## Public document follow-up — 9 September 2026

Clicking marker **3091** in the [current official GIS](https://geo.zh.ch/maps?initialMapIds=TBAVMSZH&x=2705704.39&y=1248957.73&scale=1500) exposes a **Daten der Verkehrsmessstelle / Herunterladen** link to the [station's annual report](https://maps.zh.ch/system/docs/tbavertraege/Messstellen/3091.pdf). This link was absent from the checked public WFS response. The GIS also links the station's Verkehr Online page.

The downloaded report contains **seven pages of 2025 data**, dated **6 March 2026**. All seven pages were rendered and visually inspected. Page 1 identifies Hittnau 3091, Tösstalstrasse, route 337, and includes a location map. Page 2 states **R1 = Fahrtrichtung Pfäffikon** and **R2 = Fahrtrichtung Saland**. The remaining pages contain statistical profiles, an events area and holidays. No sensor layout, direction arrows tied to loops, surveyed azimuth or mapping to feed detector 3091.02 is supplied.

The [document review](../data/hittnau-station-document-review.json) pins the exact download URL, discovery route, byte count, SHA-256 hash, data year and page-by-page findings. The report provides station and label corroboration; its annual averages are not observations for the September 2026 recording. R1/R2 must not be assumed to equal feed channels .01/.02 without independent mapping evidence. The original geometry, direction thresholds and candidate audit are unchanged.

Both the [project-document portal](https://www.notes.zh.ch/bd/tba/bdtbaerwtr.nsf) and the linked [Verkehr Online station page](https://vdp.zh.ch/spa/gis/verkehrsdaten-online/M3091) showed login forms in the inspected browser. No configured credentials for those services were found in the project. This says nothing about credentials the user may hold elsewhere or about documents behind those logins. A [publisher request draft](HITTNAU-STATION-PLAN-REQUEST.md) now specifies the missing evidence. It has not been sent.
