...
### Reviewed national test fixtures

The manual-only `Refresh national test fixtures` GitHub Actions workflow checks the official current-resource permalink for a chosen service date. Live publication uses the Pages/R2 pipeline; fixture refreshes no longer have a separate schedule. It regenerates the morning, full-day and hub studies together in an isolated runner, downloads and joins the official rail geometry, then runs `npm run data:validate`, the unit tests, typecheck, explicit fixture build and transfer budget before force-updating the dedicated `automation/refresh-national-data` review branch. The run summary links either to its open pull request or to GitHub's pre-filled comparison page for the first review. Keeping first-time PR creation manual avoids granting every repository workflow permission to approve pull requests. The committed national test fixtures therefore change only through a reviewed, validated commit; this review branch does not modify the regional studies. The daily Pages workflow independently refreshes and validates their shape-enriched morning/full-day artifacts for publication; see [EXPLORATION.md](EXPLORATION.md#automatic-publication).
