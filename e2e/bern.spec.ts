...
test('Bern loads on selection, searches rail and out-of-canton stops, seeks and shares', async ({ page }, testInfo) => {
  ...
  await search.fill('IR15')
  await page.locator('.search-results .route-result').first().click()
  await expect(page.locator('.route-card')).toContainText('IR15')
  await search.fill('RE8')
  await page.locator('.search-results .route-result').first().click()
  await expect(page.locator('.route-card')).toContainText('RE8')
  await search.fill('Interlaken Ost')
  await page.locator('.search-results .station-result').first().click()
  await expect(page.locator('.station-card')).toContainText('Interlaken Ost')
  await search.fill('Morges')
  await page.locator('.search-results .station-result').first().click()
  await expect(page.locator('.station-card')).toContainText('Morges')
  ...
  await page.locator('.search-results .route-result').first().click()
  await expect(page.locator('.route-card')).toContainText('IC61')
})
