...
test('Bern loads on selection, searches rail and out-of-canton stops, seeks and shares', async ({ page }, testInfo) => {
  ...
  await search.fill('IR66')
  await page.locator('.search-results .route-result').first().click()
  await expect(page.locator('.route-card')).toContainText('IR66')
  await search.fill('IR16')
  await page.locator('.search-results .route-result').first().click()
  await expect(page.locator('.route-card')).toContainText('IR16')
  await search.fill('S21')
  await page.locator('.search-results .route-result').first().click()
  await expect(page.locator('.route-card')).toContainText('S21')
  await search.fill('Fribourg/Freiburg')
  await page.locator('.search-results .station-result').first().click()
  await expect(page.locator('.station-card')).toContainText('Fribourg/Freiburg')
