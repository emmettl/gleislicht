...
test('Horgen afternoon playback uses its own date, hides traffic in gaps, and returns to morning', async ({ page, isMobile }) => {
  const errors: string[] = []
  ...
  await card.getByRole('button',{name:'Play Horgen recording'}).click()
