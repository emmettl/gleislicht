...
vi.mock('@motionstudies/three/NationalNetworkScene', () => ({ NationalNetworkScene: ({ snapshot }: { snapshot: NetworkSnapshot }) => <div data-testid="map-scene" data-train-count={snapshot.trains.length} data-train-ids={snapshot.trains.map(train => train.id).join(',')} /> }))
