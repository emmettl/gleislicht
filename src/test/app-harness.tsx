...
vi.mock('@motionstudies/three/NationalNetworkScene', () => ({ NationalNetworkScene: ({ snapshot, referenceSnapshot, cameraFraming }: { snapshot: NetworkSnapshot; referenceSnapshot: NetworkSnapshot; cameraFraming?: MapCameraFraming }) => <div data-testid="map-scene" data-camera-scale={cameraFraming?.homeDistanceScale} data-bounds={JSON.stringify(snapshot.bounds)} data-reference-bounds={JSON.stringify(referenceSnapshot.bounds)} data-train-count={snapshot.trains.length} data-train-ids={snapshot.trains.map(train => train.id).join(',')} /> }))
