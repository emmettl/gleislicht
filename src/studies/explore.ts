...
export const STUDY_IDS = ['national', 'postbus', 'zvv-region', 'geneva-tpg', 'zurich-city', 'rigi-lake', 'contrast', 'jungfrau', 'lausanne-region', 'basel-core', 'bern-region', 'gornergrat', 'solothurn-region', 'nyon-region', 'graubuenden-region'] as const
export const REGIONAL_DAYS = { 'graubuenden-region': 'graubuenden-region/2026-09-04/graubuenden-region-day-manifest.json', 'solothurn-region': 'solothurn-region-day-manifest.json', 'nyon-region': 'nyon-region-day-manifest.json', 'bern-region': 'bern-region-day-manifest.json', 'basel-core': 'basel-core-day-manifest.json', 'lausanne-region': 'lausanne-region-day-manifest.json', 'zvv-region': 'zvv-region-day-manifest.json', 'geneva-tpg': 'geneva-tpg-day-manifest.json', 'zurich-city': 'zurich-city-day-manifest.json' } as const
...
export function readStudyLink(search: string): StudyLink {
  ...
  return { study, range: p.get('range') === 'day' || (['graubuenden-region', 'lausanne-region', 'basel-core', 'bern-region', 'solothurn-region', 'nyon-region'].includes(study) && p.get('range') !== 'morning') ? 'day' : 'morning', time: Number.isFinite(time) && time >= 0 && time < 86400 ? time : undefined, date: /^\d{4}-\d{2}-\d{2}$/.test(date) ? date : undefined, station: p.get('station')?.slice(0, 200) || undefined, train: p.get('train')?.slice(0, 200) || undefined }
}
