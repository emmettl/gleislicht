/* oxlint-disable react/immutability -- The renderer owns this mutable Three camera; the flight driver updates it before drawing. */
import { useSyncExternalStore } from 'react'
import type { NetworkCameraContext, NetworkCameraDriver } from '@motionstudies/three/scene-extensions'
import { mapCameraFieldOfView } from '@motionstudies/three/map-camera'
import { fromOrbit, orbitalFlight, toOrbit, type Point3 } from './orbital-flight.ts'
...
export function createAtlasFlightCamera({ camera, target, projection }: NetworkCameraContext): NetworkCameraDriver {
  ...
  return {
    dispose() { orbitalFlight.atlas.delete(handle) },
    update() {
