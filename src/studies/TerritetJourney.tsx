...
export default function TerritetJourney({network,language,time,onSeek,onFollow,onFinish,onExit,onTerrain,terrainUrl}:{network:NetworkSnapshot;language:UiLanguage;time:number;onSeek:(time:number)=>void;onFollow:(id:string|undefined,station:string|undefined)=>void;onFinish:()=>void;onExit:()=>void;onTerrain:(binding:MeasuredTerrainBinding|undefined)=>void;terrainUrl:string}){
 const copy=TERRITET_COPY[language],stops=useRef<HTMLElement>(null),[direction,setDirection]=useState<TerritetDirection>('ascent'),[id,setId]=useState<string>(),[expanded,setExpanded]=useState(false)
 const choices=useMemo(()=>territetJourneys(network,direction),[network,direction])
 const train=choices.find(t=>t.id===id)??choices.reduce((a,b)=>Math.abs(b.start-43200)<Math.abs(a.start-43200)?b:a,choices[0])
 const terrainCopy=territetTerrainCopy(language)
 ...
 useEffect(()=>{
  ...
  fetch(terrainUrl,{signal:controller.signal}).then(response=>{if(!response.ok)throw new Error('Terrain unavailable');return response.json()}).then(value=>{if(!controller.signal.aborted)setTerrainData({value})}).catch(()=>{if(!controller.signal.aborted)setTerrainError(true)})
  return()=>controller.abort()
 },[terrainWanted,terrainData,attempt,terrainUrl])
 const terrain=useMemo(()=>train && terrainData?bindTerritetTerrain(terrainData.value,network,train):undefined,[terrainData,network,train])
 ...
 return <section className="journey-card territet-journey" aria-label={copy[direction]} data-phase={phase} data-direction={direction} data-expanded={expanded} data-compact={terrainWanted && !expanded} data-terrain-wanted={terrainWanted}>
  <header><h2>{copy[direction]}</h2><button className="territet-details-toggle" aria-label={copy.details} aria-expanded={expanded} onClick={()=>setExpanded(v=>!v)}>≡</button><button onClick={onExit} aria-label={copy.exit}>×</button></header>
  <label>{copy.direction}<select value={direction} onChange={e=>{setDirection(e.target.value as TerritetDirection);setId(undefined)}}><option value="ascent">{copy.ascent}</option><option value="descent">{copy.descent}</option></select></label>
  {!train?<p role="status">{copy.empty}</p>:<>
   ...
   <p role="status">{phase==='complete'?`${copy.arrived} · ${to}`:phase==='before'?copy.before:terrainVisible?terrainCopy.outdoor:copy.model}</p>
      <div className="territet-terrain-control">
        <button className="territet-terrain-button" type="button" aria-pressed={terrainWanted} onClick={() => { setTerrainError(false); setTerrainWanted(value => !value) }}>{terrainWanted ? terrainCopy.hide : terrainCopy.show}</button>
        ...
        {terrainWanted && <p role="status" data-terrain-status={failed ? 'error' : !terrain ? 'loading' : terrainVisible ? 'outdoor' : terrainPosition?.mask?.reason ?? 'wait'}>{failed ? terrainCopy.error : !terrain ? terrainCopy.loading : phase==='complete' ? `${copy.arrived} · ${to}` : phase==='before' ? copy.before : terrainVisible ? terrainCopy.outdoor : terrainPosition?.mask?.kinds.includes('Ground clearance')?terrainCopy.clearance:terrainCopy[terrainPosition?.mask?.reason ?? 'wait']}</p>}
        {terrainWanted && failed && <button className="territet-terrain-button" onClick={() => { setTerrainError(false); setTerrainData(undefined); setAttempt(n => n+1) }}>{terrainCopy.retry}</button>}
        {terrainWanted && terrain && <details><summary>{terrainCopy.sources}{railHeight !== undefined ? ` · ${railHeight.toLocaleString(language)} m` : ''}</summary><p>{terrainCopy.model}</p>{railHeight !== undefined && <p>{terrainCopy.elevation}: {railHeight.toLocaleString(language)} m (LN02)</p>}<p>© swisstopo · swissALTI3D {terrain.data.metadata.terrainRelease} · swissTLM3D 2026-02</p><a href="./methodology.html#territet-terrain" target="_blank" rel="noreferrer">{terrainCopy.sources} ↗</a></details>}
      </div>
   <p>{copy.serviceNote}</p>
   <details><summary>{copy.evidence}</summary><p>{copy.note}</p><a href="./methodology.html#territet" target="_blank" rel="noreferrer">{copy.evidence} ↗</a> · <a href="https://www.mob.ch/en/stories/territet-glion-funicular" target="_blank" rel="noreferrer">MVR ↗</a></details>
