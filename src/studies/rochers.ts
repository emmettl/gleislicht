...
export type RochersDirection = 'ascent' | 'descent'
export const rochersAscents = (network:NetworkSnapshot) => rochersJourneys(network,'ascent')
export const rochersDescents = (network:NetworkSnapshot) => rochersJourneys(network,'descent')
export function rochersJourneys(network:NetworkSnapshot, direction:RochersDirection) {
 ...
 return network.trains.filter(train=>{
  ...
  if(actual.routeId!=='91-37-F-j26-1' || actual.agencyId!=='131' || actual.routeType!==106 || !source || source.calls.length<2 || source.calls.length!==train.stops.length)return false
  const first=source.calls[0],last=source.calls.at(-1)!
  if(first.pickup!=='0' || last.dropOff!=='0' || root(network.stops[train.stops[0][0]]?.[4])!==(direction==='ascent'?'ch:1:sloid:1300':'ch:1:sloid:1369') || root(network.stops[train.stops.at(-1)![0]]?.[4])!==(direction==='ascent'?'ch:1:sloid:1369':'ch:1:sloid:1300'))return false
