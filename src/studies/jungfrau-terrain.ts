...
export type JungfrauTerrainRoute = { routeId: string; legIndex: number; vehicle: 'train' | 'cogwheel'; points: TerrainPoint[]; stops: { id: string; name: string; progress: number }[]; maskedRanges: { start: number; end: number; names: string[]; kinds: string[]; reason: 'tunnel' | 'covered' | 'alignment' }[] }
export type JungfrauTerrain = {
 id: 'jungfrau-ascent-terrain'; version: 1
 ...
 routes: JungfrauTerrainRoute[]
}
export type TerrainWindow = { start: number; end: number; legIndex: number }
export type JungfrauTerrainBinding = { data: JungfrauTerrain; routes: { route: JungfrauTerrainRoute; calls: { name: string; arrival: number; departure: number; progress: number }[] }[]; windows: TerrainWindow[] }
const root = (id: string | undefined) => id?.match(/^ch:1:sloid:\d+/)?.[0]
const visible = (route: JungfrauTerrainRoute, progress: number) => !route.maskedRanges.some(r => progress>=r.start && progress<=r.end)
export function bindJungfrauTerrain(value: unknown, network: NetworkSnapshot, sequence: JungfrauAscent): JungfrauTerrainBinding | undefined {
 try {
 const data=value as JungfrauTerrain | undefined
 const metadata=network.metadata as NetworkSnapshot['metadata'] & {sources?: {timetable?: {sha256?: string}}}
 if (!data || data.id!=='jungfrau-ascent-terrain' || data.version!==1 || data.metadata?.serviceDate!==metadata.serviceDate || data.metadata?.feedVersion!==metadata.feedVersion || !metadata.sources?.timetable?.sha256 || data.metadata?.timetableSha256!==metadata.sources.timetable.sha256) return
 ...
 if(!Array.isArray(data.routes) || data.routes.length!==3) return
 const routes: JungfrauTerrainBinding['routes']=[], windows: TerrainWindow[]=[]
 for(let i=0;i<3;i++) {
  const leg=sequence.legs[i],route=data.routes[i],actual=leg?.train as typeof leg.train & {routeId?:string}
  if(!leg || route.legIndex!==i || route.routeId!==actual.routeId || route.vehicle!==(i===0?'train':'cogwheel') || !Array.isArray(route.points) || route.points.length<2 || route.points.length>10000 || route.points.some(p=>!Array.isArray(p) || p.length!==3 || !p.every(Number.isFinite) || Math.abs(p[0])>grid.widthMetres/2 || Math.abs(p[1])>grid.depthMetres/2 || p[2]<0 || p[2]>4500)) return
  const distances=railSamples(route.points)
  if(distances.some((d,j)=>j>0 && d<=distances[j-1]))return
  if(!Array.isArray(route.stops) || route.stops.length!==leg.train.stops.length || route.stops[0].progress!==0 || route.stops.at(-1)!.progress!==1 || route.stops.some((s,j)=>!root(s.id) || root(s.id)!==root(network.stops[leg.train.stops[j][0]]?.[4]) || !Number.isFinite(s.progress) || s.progress<0 || s.progress>1 || j>0 && s.progress<=route.stops[j-1].progress)) return
  ...
  const calls=leg.train.stops.map(([,arrival,departure],j)=>({name:route.stops[j].name,arrival,departure,progress:route.stops[j].progress}))
  ...
  for(let j=0;j<calls.length-1;j++) {
   ...
   if(a.arrival>a.departure || a.departure>=b.arrival)return
...
export function jungfrauTerrainPosition(binding: JungfrauTerrainBinding, time: number) {
