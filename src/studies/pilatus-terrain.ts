...
import { pilatusAscents, pilatusDescents } from './pilatus.ts'
import { bindMeasuredTerrain, type MeasuredTerrain } from './measured-terrain.ts'
export function bindPilatusTerrain(value: unknown, network: NetworkSnapshot, train: NetworkSnapshot['trains'][number]) {
 const ascent=pilatusAscents(network).includes(train)
 if(!ascent && !pilatusDescents(network).includes(train))return
 try {
  ...
  return bindMeasuredTerrain(oriented,network,{id:'pilatus-ascent-terrain',legs:[{train,departure:train.stops[0][2],arrival:train.stops.at(-1)![1],vehicle:'cogwheel'}]})
