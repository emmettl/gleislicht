...
export function gornergratAscents(network:NetworkSnapshot) {
 ...
 return network.trains.filter(train=>{
  ...
  if(first.pickup!=='0' || last.dropOff!=='0' || network.stops[train.stops[0][0]]?.[2]!=='Zermatt GGB' || network.stops[train.stops.at(-1)![0]]?.[2]!=='Gornergrat')return false
