...
export type GornergratDirection = 'ascent' | 'descent'
export const gornergratAscents = (network:NetworkSnapshot) => gornergratJourneys(network,'ascent')
export const gornergratDescents = (network:NetworkSnapshot) => gornergratJourneys(network,'descent')
export function gornergratJourneys(network:NetworkSnapshot, direction:GornergratDirection) {
 ...
 return network.trains.filter(train=>{
  ...
  if(actual.routeId!=='93-48-j26-1' || actual.agencyId!=='121' || actual.routeType!==116 || !source || source.calls.length<2 || source.calls.length!==train.stops.length)return false
  const first=source.calls[0],last=source.calls.at(-1)!
  if(first.pickup!=='0' || last.dropOff!=='0' || root(network.stops[train.stops[0][0]]?.[4])!==(direction==='ascent'?'ch:1:sloid:1690':'ch:1:sloid:1694') || root(network.stops[train.stops.at(-1)![0]]?.[4])!==(direction==='ascent'?'ch:1:sloid:1694':'ch:1:sloid:1690'))return false
