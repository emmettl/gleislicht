...
import { jungfrauTerrainPosition, railPoint, railSamples, type JungfrauTerrainBinding, type TerrainWindow } from './jungfrau-terrain.ts'

const SCALE = 650
function Landscape({ binding, time, onReady }: { binding: JungfrauTerrainBinding; time: number; onReady: () => void }) {
  ...
  const position=jungfrauTerrainPosition(binding,time)!
...
export default function JungfrauTerrainScene({ binding, window: activeWindow, time, isPlaying, rate, onTime }: {
  binding: JungfrauTerrainBinding; window: TerrainWindow; time: number; isPlaying: boolean; rate: number; onTime: (time:number)=>void
}) {
  ...
  return <div className="jungfrau-terrain-scene" data-rendered={ready} data-progress={position.progress} data-stopped={position.stopped} data-leg={position.legIndex} style={{width:'100%',height:'100%'}}>
