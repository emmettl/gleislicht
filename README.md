...
Use the **CH / 24H / ZVV / ZH** switch in the search bar to move between Switzerland's rail atlas, its full day, the ZVV region and Zürich city. All studies share the national coordinate system: selecting ZVV or ZH physically flies and zooms the camera into the chosen area while a dim national rail layer preserves geographic context, and CH reverses the journey. Reset always returns to the active study's home view. The regional and city artifacts remain separate on-demand downloads, so neither enlarges the national first load. Tram and bus routes, vehicles, trails and selections follow geometry joined from the official ZVV feed; unmatched services retain honest straight stop interpolation. See [docs/REGIONAL-STUDIES.md](./docs/REGIONAL-STUDIES.md) for the ZVV, Genève/TPG and rural PostBus direction.

The ZVV overview deliberately opens with a rail-led hierarchy: weighted local corridors remain visible, individual trams reveal after the first zoom descent, and buses appear closer in. Selecting any local category, line, station or vehicle immediately overrides that aggregation so search and comparison remain direct.
...
The committed GTFS and boundary snapshots are regenerated with `npm run data:gtfs`, `npm run data:day`, `npm run data:zvv`, `npm run data:zvv:shapes`, `npm run data:zurich`, `npm run data:zurich:shapes` and `npm run data:boundary`; see [docs/DATA-PIPELINE.md](./docs/DATA-PIPELINE.md).
