...
Motion Studies 006 now has a non-public rail-led adapter suite. `npm run data:london:proofs` compiles source-audited Bakerloo, Northern, DLR, Tramlink, Lioness and Elizabeth line weekday movements into the same compact network contract used by Gleislicht, including dwell normalisation, short turns and branch-specific route geometry. Because TfL's documented Journey Planner timetable feed excludes Overground and Elizabeth line, their bounded proofs use hash-audited current TfL timetable PDFs and accept only complete monotonic grid columns. `npm run data:london:assemble` merges 230 representative journeys and 151 stops across all five modes, while `npm run data:london:catalogue` preserves all 20 current line identities, 125 directional branches and representative NaPTAN platform/interchange hierarchies. Everything remains outside the Swiss public payload and selector. See [docs/LONDON.md](./docs/LONDON.md).
