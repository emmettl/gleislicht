...
**AUTO** completes the first transport-data triad with a separately loaded A1 road study through Zürich. Warm-white light traffic and larger amber heavy vehicles are synthetic particles reconstructed from directional flow and mean speed (`density = flow / speed`), not tracked automobiles. Road Study 001 uses real ASTRA detector geography and a clearly disclosed representative morning calibration. Selecting AUTO reveals a separately loaded official national-road skeleton, 609 measurement-ready counter sections and searchable A-road corridor focus, backed by a reproducible audit of 458 federal counter sites. The repository also includes an authenticated one-minute recorder and a completeness-gated compiler for moving to genuine measurements without changing the browser contract. See [docs/AUTO.md](./docs/AUTO.md).
...
Motion Studies 006 now has a dedicated **All Change** preview at `/london.html`. It shares Gleislicht's renderer, map camera, playback, timetable interpolation, station interaction and selection semantics, but owns its title, theme, geography adapter, data catalogue and page metadata. Its complete initial view—including the immediately loaded 3D renderer and the 1,743-movement, 503-stop rail study—stays below a 650 KiB compressed transfer ceiling. Build-time staging keeps the canonical London artifacts in fixtures, prevents duplicate repository blobs and leaves every London file outside the Swiss page's request graph and selector. **GEOGRAPHY / DIAGRAM** now transforms the moving network continuously between real coordinates and a deterministic octilinear study without resetting the clock or selection. The 503-stop, 1,395-path diagram is a separate sub-10 KiB compressed download and deliberately avoids reproducing TfL's map artwork. See [docs/LONDON.md](./docs/LONDON.md).
...
See [ROADMAP.md](./ROADMAP.md) for delivery stages, [docs/VISION.md](./docs/VISION.md) for the product and art direction, and [docs/EXTRACTION.md](./docs/EXTRACTION.md) for the eventual `@motionstudies` package/repository seam.
