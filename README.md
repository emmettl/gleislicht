...
Motion Studies 006 now has a non-public rail-led adapter suite. `npm run data:london:proofs` compiles source-audited Bakerloo, Northern, DLR and Tramlink weekday movements into the same compact network contract used by Gleislicht, including dwell normalisation, short turns and branch-specific route geometry. `npm run data:london:catalogue` inventories all 20 current lines across Underground, Elizabeth line, London Overground, DLR and Tramlink, preserving 125 directional branches plus representative NaPTAN platform/interchange hierarchies. These fixtures stay outside the Swiss public payload and selector. Elizabeth line and Overground remain explicitly topology-only until their missing tested Unified API timetable endpoints are replaced by a verified source. See [docs/LONDON.md](./docs/LONDON.md).
