...
Motion Studies 006 now has a non-public rail-led morning compiler. `npm run data:london:lattice:unified` expands every advertised Tube, DLR and Tramlink line from all 48 directional branch origins, retaining limited-stop geometry and recording two topology-only Metropolitan origins with no Friday schedule. The resulting study contains 1,422 movements across 13 lines and 26 directions. Because TfL's documented Journey Planner timetable feed excludes Overground and Elizabeth line, `npm run data:london:lattice:pdf` audits all 43 advertised branches against the current, hash-recorded TfL timetable PDFs. All six named Overground lines have active movements in both directions and 16 of 19 Elizabeth branch families compile; the seven unmatched topology branches remain explicit. `npm run data:london:assemble` combines 1,743 movements and 503 stops across all five rail-led modes, then freezes a 460-name label hierarchy from interchange modes, distinct lines, calls and graph connectivity. `npm run data:london:geography` compiles the GLA's Greater London and River Thames polygons from 17,112 source points into a 24 KiB rendering artifact. Everything remains outside the Swiss public payload and selector. See [docs/LONDON.md](./docs/LONDON.md).
