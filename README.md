...
Motion Studies 006 now has a dedicated **All Change** preview at `/london.html`. It shares Gleislicht's renderer, map camera, playback, timetable interpolation, station interaction and selection semantics, but owns its title, theme, geography adapter, data catalogue and page metadata. Its complete initial view—including the immediately loaded 3D renderer and rail study—stays below a 650 KiB compressed transfer ceiling. Build-time staging keeps canonical London rail artifacts in fixtures and leaves every London file outside the Swiss page's request graph and selector. **GEOGRAPHY / DIAGRAM** transforms the moving network continuously between real coordinates and a deterministic octilinear study without resetting the clock or selection. An optional **AIR** layer now adds matching two-hour and progressive 24-hour observed-aircraft studies, with callsign/ICAO search, category isolation, labels and altitude-aware follow views; it remains completely outside the opening request graph. See [docs/LONDON.md](./docs/LONDON.md).
